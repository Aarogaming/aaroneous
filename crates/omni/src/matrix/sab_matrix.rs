        ...
        self.manifest_paths()?
            ...
            .map(|path| {
                let content = fs::read_to_string(&path).with_context(|| {
                    format!("Failed to read SAB manifest at {}", path.display())
                })?;
                let manifest =
                    serde_json::from_str::<SabManifest>(&content).with_context(|| {
                        format!("Failed to parse SAB manifest at {}", path.display())
                    })?;
                Ok(manifest)
            })
...
impl SabMatrix {
    ...
    pub fn load_cached_from_registry_dir(registry_dir: impl AsRef<Path>) -> Result<Option<Self>> {
        ...
        let content = fs::read_to_string(&generated_path).with_context(|| {
            format!(
                "Failed to read generated SAB matrix at {}",
                generated_path.display()
            )
        })?;
        let matrix = serde_json::from_str::<SabMatrix>(&content).with_context(|| {
            format!(
                "Failed to parse generated SAB matrix at {}",
                generated_path.display()
            )
        })?;
    ...
    pub fn save_generated_to(&self, registry_dir: impl AsRef<Path>) -> Result<PathBuf> {
        ...
        fs::write(&generated_path, content).with_context(|| {
            format!(
                "Failed to write generated SAB matrix at {}",
                generated_path.display()
            )
        })?;
        Ok(generated_path)
    ...
    pub fn refresh_generated_cache_from_registry_dir(
    ...
    pub fn upsert_surface(&mut self, surface: SabSurface) {
        if let Some(existing) = self
            .surfaces
            .iter_mut()
            .find(|existing| existing.name == surface.name)
        {
            *existing = surface;
        } else {
...
mod tests {
  ...
  "registry_patterns": ["monitoring_agent"]
    ...
    fn saves_and_loads_generated_matrix() {
        ...
        let loaded = SabMatrix::load_cached_from_registry_dir(&temp_dir)
            .unwrap()
            .unwrap();
        assert_eq!(
            loaded.surface_for_artifact("cached.wasm").unwrap().name,
            "cached_surface"
        );
        ...
        assert_eq!(
            round_trip
                .surface_for_pattern("cached_pattern")
                .unwrap()
                .best_fit_module,
            "src/test.rs"
        );
  ...
  "registry_patterns": ["new_feature_pattern"]
        ...
        assert_eq!(
            loaded
                .surface_for_pattern("new_feature_pattern")
                .unwrap()
                .best_fit_module,
            "src/new_feature.rs"
        );
  ...
  "registry_patterns": ["refresh_pattern"]
        ...
        let matrix =
            SabMatrix::refresh_generated_cache_from_registry_dir(temp_dir.clone()).unwrap();
        assert_eq!(
            matrix.surface_for_artifact("refresh.wasm").unwrap().name,
            "refresh_surface"
        );
