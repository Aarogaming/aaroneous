...
impl ThermodynamicGovernor {
    ...
    pub fn predict_metabolic_risk(&mut self) -> ThermodynamicForecast {
        ...
        let prediction_error =
            (estimated_load - self.config.target_free_energy).abs() * self.config.error_weight;
        ...
        match forecast.phase {
            ...
            SystemPhase::Ordered => {
                ...
                if forecast.risk_score < 0.3 && biology.expression_rate < 1.0 {
                    ...
                    ThermodynamicAction::StableExploitation {
                        forecast: forecast.clone(),
                    }
                }
            }
            ...
            SystemPhase::Unknown => ThermodynamicAction::InsufficientData {
                forecast: forecast.clone(),
            },
        }
    }
