...
impl McpGateway {
    ...
    pub fn translate_inbound(json_rpc: &str, frame: &mut McpToolCallFrame) -> Result<()> {
        ...
        if arg_bytes.len() > 2048 {
            return Err(anyhow!(
                "MCP arguments exceed mmap buffer size (2048 bytes)"
            ));
        }
        ...
        println!(
            "[McpGateway] Inbound translation complete for tool: {}",
            tool_name
        );
        Ok(())
    }
    ...
    pub fn translate_outbound(frame: &McpToolCallFrame) -> Result<String> {
        if frame.status != 3 && frame.status != 4 {
            return Err(anyhow!(
                "Tool execution not complete (Status: {})",
                frame.status
            ));
        }

        let result_str =
            std::str::from_utf8(&frame.arguments_payload[..frame.arguments_size as usize])
