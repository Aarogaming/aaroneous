...
fn main() -> Result<()> {
    ...
    let target_dir: PathBuf = env::args()
        .nth(1)
        .unwrap_or_else(|| "dev/canary_legacy_fixture/".to_string())
        .into();
...
fn generate_markdown_report(report: &BatchReport) {
    ...
    for (name, status) in &report.per_crate_status {
        let compliant_icon = if status.compliant {
            "(OK)"
        } else {
            "(NEEDS FIX)"
        };
        println!(
            "| {} | {} | {} | {}",
            name, status.files_inspected, status.violations_found, compliant_icon
        );
    }
