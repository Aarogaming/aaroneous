...
use orchestration_plane::normalization_pipeline::{InvariantSeverity, NormalizationPipeline};
use std::path::Path;
...
VERIFYING DELIBERATE VIOLATIONS:"
    ...
    let has_unwrap = plan
        .violations
        .iter()
        .any(|v| v.description.contains("unwrap"));
    let has_panic = plan
        .violations
        .iter()
        .any(|v| v.description.contains("panic!"));
    let has_unsafe = plan
        .violations
        .iter()
        .any(|v| v.description.contains("unsafe"));
