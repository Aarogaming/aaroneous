...
use mcp_server::action_executor::{ActionExecutor, ActionResult, ExecutionStats};
use mcp_server::decision_engine::{
    AutonomousDecisionEngine, DecisionTask, ExecutionOutcome, TaskEvaluation,
};
use mcp_server::intelligence::{IntelligenceEngine, LLMConfig, ProviderType, Specialist, TaskType};
use mcp_server::metadata_ingestor::{
        ...
        for (evaluation, outcome) in report.evaluations.iter().zip(report.outcomes.iter()) {
            if let ExecutionOutcome::Completed { .. } = outcome {
                ...
                if let Some(exec_action) = action {
                    let _result = self.executor.execute(exec_action).await;
...
mod tests {
    ...
    #[test]
    fn test_daemon_shutdown_sets_state() {
        let config = OrchestrationDaemonConfig::default();
