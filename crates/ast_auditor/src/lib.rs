...
/// Recursively traverses targeted paths, executes AST and manifest audits, and returns success/failure.
    ...
    P: AsRef<Path>,
{
    ...
    if report.has_failures() {
        eprintln!("\nAUDIT GATE FAILED: Repository invariant violations detected.");
        Err(ExitCode::FAILURE)
    } else {
        println!("\nAUDIT GATE PASSED: All architectural AST invariants verified.");
        Ok(())
    }
}
