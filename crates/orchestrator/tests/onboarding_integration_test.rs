...
use ipc_bus::universal_protocol::{
    ...
    UcpBroadcastType, UcpRequestType, UniversalClientRequest,
};
use orchestrator::onboarding::{
    handle_onboarding_event, process_onboarding_request, ComponentOnboardingTask,
    handle_assimilation_event, process_client_request, AssimilationTask, Idle, Synthesizing,
};
...
fn test_e2e_universal_client_request_ingestion() {
    ...
    let broadcast = process_client_request(&req).expect("Client request must process into broadcast");
    assert_eq!(broadcast.broadcast_type, UcpBroadcastType::AssimilationState as u32);
...
fn test_e2e_certification_retry_loop_and_rejection() {
    ...
    let task1 = AssimilationTask::<Synthesizing> { record, _marker: PhantomData };
    let certifying1 = task1.finalize_synthesis();
    let retry1 = certifying1.certify(false, max_retries).expect_err("certify must fail").expect("retry 1");
    ...
    let retry2 = certifying2.certify(false, max_retries).expect_err("certify must fail").expect("retry 2");
    ...
    let retry3 = certifying3.certify(false, max_retries).expect_err("certify must fail").expect("retry 3");
    ...
    let rejected = certifying4.certify(false, max_retries).expect_err("certify must fail").expect_err("exhausted");
...
fn test_e2e_event_bus_zero_copy_dispatch() {
    ...
    let envelope = subscriber.try_recv().expect("Subscriber must receive envelope");
    ...
    let transitioned = handle_assimilation_event(&envelope.payload).expect("Reducer must process event");
