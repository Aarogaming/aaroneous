...
/// Visitor that inspects AST items and string literals for `aaroneous_*` stutter.
...
impl<'a> PrefixStutterVisitor<'a> {
    ...
    pub fn is_exempt_metric_identifier(s: &str) -> bool {
        if !s.starts_with("aaroneous_") && !s.starts_with("aaroneous.") {
            return false;
        }
    ...
    fn check_identifier(&mut self, ident: &Ident, kind: &str) {
        ...
        if name.starts_with("aaroneous_") || name.starts_with("Aaroneous") {
            self.push_violation(
                ident,
                &name,
                &format!("Remove `aaroneous_` prefix stutter from {kind}. Use concise generic systems nomenclature."),
            );
        }
    }
}
...
impl<'ast> Visit<'ast> for PrefixStutterVisitor<'_> {
    ...
    fn visit_lit_str(&mut self, node: &'ast LitStr) {
        ...
        let val = node.value();
        if val.starts_with("aaroneous_") || val.starts_with("aaroneous/") || val.starts_with("aaroneous-") {
            ...
            self.push_violation(
                node,
                &format!("\"{val}\""),
                "IPC channel, named pipe, shared-memory identifier, or binary moniker contains `aaroneous_` stutter. Strip prefix.",
            );
        }
        ...
        if let Some(name) = package.get("name").and_then(|v| v.as_str()) {
            if name.starts_with("aaroneous_") || name.starts_with("aaroneous-") {
                violations.push(PrefixStutterViolation {
                    ...
                    remediation: format!("Rename crate to strip prefix stutter (e.g. `{}`)", name.trim_start_matches("aaroneous_").trim_start_matches("aaroneous-")),
                });
            }
        }
    }
    ...
    for table_key in dep_tables {
        ...
        if let Some(table) = table_opt.and_then(|t| t.as_table()) {
            for (dep_name, dep_val) in table {
                ...
                if is_internal && (dep_name.starts_with("aaroneous_") || dep_name.starts_with("aaroneous-")) {
                    violations.push(PrefixStutterViolation {
                        file_path: pbuf.clone(),
                        line: 1,
                        column: 1,
