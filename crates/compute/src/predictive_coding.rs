...
impl PredictiveNode {
    ...
    pub fn adapt_precision(&mut self, decay: f64) {
        ...
        self.precision =
            (self.precision * (1.0 - decay) + (1.0 - error_ratio) * decay).clamp(0.01, 10.0);
    }
}
...
impl HierarchicalPredictiveCoding {
    ...
    pub fn process(&mut self, observation: &[f64]) -> f64 {
        ...
        for layer_idx in 1..self.layers.len() {
            ...
            for (node_idx, node) in current_layer.iter_mut().enumerate() {
                if layer_idx - 1 < self.top_down_weights.len()
                    && node_idx < self.top_down_weights[layer_idx - 1].len()
                {
                    // Top-down prediction
    ...
    pub fn get_prediction_errors(&self) -> Vec<Vec<f64>> {
        self.layers
            .iter()
            .map(|layer| layer.iter().map(|n| n.prediction_error).collect())
            .collect()
    }
}
