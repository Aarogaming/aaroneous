...
fn test_rls_division_by_zero_protection() {
    ...
    assert!(!factor.is_nan(), "RLS update produced NaN from near-zero covariance");
...
fn test_rls_positive_definiteness_invariant() {
    ...
    assert!(covariance.p00 > 0.0, "Diagonal element still negative after enforcement");
}
...
fn test_rls_nan_propagation_prevention() {
    ...
    assert!(!result.unwrap().p00.is_nan(), "NaN still present after sanitization");
}
...
fn test_rls_infinity_prevention() {
    ...
    assert!(!result.unwrap().p00.is_infinite(), "Infinity still present after clamping");
}
...
fn sanitize_covariance(cov: &CovarianceMatrix) -> Result<CovarianceMatrix, String> {
    ...
    if sanitized.p00.is_nan() { sanitized.p00 = 0.0; }
    if sanitized.p11.is_nan() { sanitized.p11 = 0.0; }
    
    // Replace Inf with large finite value
    if sanitized.p00.is_infinite() { sanitized.p00 = 1e8; }
    if sanitized.p11.is_infinite() { sanitized.p11 = 1e8; }
