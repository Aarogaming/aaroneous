...
use super::verifier::{ConstellationUpdate, VerificationResult};
...
impl ScientificPipeline {
    ...
    fn generate_summary(
        ...
        let verified = verifications
            .iter()
            .filter(|v| matches!(v.verdict, super::verifier::Verdict::Verified))
            .count();
        let falsified = verifications
            .iter()
            .filter(|v| matches!(v.verdict, super::verifier::Verdict::Falsified))
            .count();
        let unstable = verifications
            .iter()
            .filter(|v| matches!(v.verdict, super::verifier::Verdict::Unstable))
            .count();
        let inconclusive = verifications
            .iter()
            .filter(|v| matches!(v.verdict, super::verifier::Verdict::Inconclusive))
            .count();
