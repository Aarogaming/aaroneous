...
fn parse_rust_ast(content: &str) -> Vec<CodeStructure> {
    ...
    for cap in func_re.captures_iter(content) {
        ...
        let end_line =
            find_closing_brace(content, cap.get(0).unwrap().end()).unwrap_or(start_line + 10);
...
fn parse_python_ast(content: &str) -> Vec<CodeStructure> {
    ...
    for cap in func_re.captures_iter(content) {
        ...
        let start_line = content[..cap.get(0).unwrap().start()].matches('\n').count() + 1;
        let end_line =
            find_python_block_end(content, cap.get(0).unwrap().end()).unwrap_or(start_line + 10);
...
fn parse_parameters(params_str: &str) -> Vec<Parameter> {
    if params_str.trim().is_empty()
        || params_str.trim() == "self"
        || params_str.trim() == "&self"
        || params_str.trim() == "&mut self"
    {
        return Vec::new();
    }
...
mod tests {
    ...
    fn test_parse_rust_function() {
        ...
        writeln!(
            file,
            "pub fn test_function(x: i32, y: String) -> Result<(), Error> {{"
        )
        .unwrap();
