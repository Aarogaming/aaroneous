//! Integration tests for the unified learning loop.
//!
//! Tests the complete OBSERVE -> ESTIMATE -> PREDICT -> ROUTE -> ACT -> LEARN
//! cycle. Restored from the pre-split `core/hypervisor/src/integration_tests.rs`
//! (see git history at `9f24821b`) and rewired across the hive_mind/host
//! crate boundary: `hypervisor::spectral_layout`, `hypervisor::tensor_router`
//! stayed in `hypervisor`; `hive_mind::unified_learning` moved to
//! `hive_mind`. `hive_mind` already takes `hypervisor` and `compute` as
//! normal (non-dev) dependencies, so this lives as a `hive_mind` integration
//! test.
...
use hypervisor::spectral_layout::spectral_layout_2d;
use hypervisor::tensor_router::{RoutingWeights, TaskEmbedding, TensorRouter};
