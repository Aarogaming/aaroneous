...
//! cross-registry synchronization maintains consistency during state
//! transitions.
//!
//! Restored from the pre-split `core/hypervisor/src/registry_sync_tests.rs`
//! (see git history at `9f24821b`) and rewired the same way as
//! `phase_6d_integration_tests.rs`: `hive_mind::hybrid_master_registry`
//! moved to `hive_mind`, and re-exports the `hypervisor::registry` types
//! (`crate::registry` pre-split) that stayed in `hypervisor`.
