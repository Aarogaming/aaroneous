...
use hypervisor::state_snapshot::{NodeMetrics, SpatialCanvasState};
...
pub enum ExecutableAction {
    ...
    ExecuteMicroBytecode {
        program: hypervisor::micro_vm::VmProgram,
...
impl ActionExecutor {
    ...
    fn execute_micro_bytecode(
        ...
        let gas = gas_limit.unwrap_or(hypervisor::micro_vm::DEFAULT_GAS_LIMIT);
        let mut vm = hypervisor::micro_vm::MicroBytecodeVm::with_limits(
            gas,
            hypervisor::micro_vm::DEFAULT_MEMORY_LIMIT,
        );
        let start = std::time::Instant::now();
...
mod tests {
    ...
    fn test_micro_bytecode_action_execution() -> Result<(), Box<dyn std::error::Error>> {
        let mut executor = ActionExecutor::new(PathBuf::from("test.wasm"));
        let program = hypervisor::micro_vm::VmProgram::new(vec![
            hypervisor::micro_vm::VmInstruction::MovImm { dst: 0, val: 100 },
            hypervisor::micro_vm::VmInstruction::MovImm { dst: 1, val: 250 },
            hypervisor::micro_vm::VmInstruction::Add { dst: 0, src: 1 },
            hypervisor::micro_vm::VmInstruction::Halt,
        ]);
