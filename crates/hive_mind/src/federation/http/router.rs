...
use hypervisor::metrics_aggregator::MetricsAggregator;
use hypervisor::rate_limit::{
...
fn workspace_models_inbox_dir() -> std::path::PathBuf {
...
// ====================================================================
// Specialist state endpoints (for SSE / XR / real-time clients)
// ====================================================================
...
///
/// Designed for persistent connection by the SSE client.
/// Pushes events on:
...
/// ```
/// Internal engine client example (conceptual):
/// ```cpp
...
async fn stream_specialists(
    ...
    let rx = state.federation.subscribe_specialist_events();
...
///   "name": "CodeReviewer",
///   "domain": "code_review",
///   "gguf_path": "/path/to/models/qwen-code-1.5b.gguf"
/// }
/// ```
...
async fn reload_dynamic_specialists(State(state): State<AppState>) -> impl IntoResponse {
    ...
    let registry_path_buf = workspace_specialist_registry_path();
    let registry_path = registry_path_buf.as_path();
...
/// ```json
/// {
///   "output_path": "/path/to/models/hybrid-v1.gguf",
///   "recipe": {
///     "recipe_id": "hybrid-v1",
///     "segments": [
///       {"source_gguf": "/path/to/models/qwen-a.gguf", "tensor_name": "blk.0.attn_q.weight"},
///       {"source_gguf": "/path/to/models/qwen-b.gguf", "tensor_name": "blk.0.mlp_gate.weight"}
///     ],
...
async fn distillation_generate(
    ...
    use llm_gateway::{LLMClient, LLMConfig};
    ...
    #[allow(unreachable_code)]
    {
        ...
        let llm_config = LLMConfig {
            provider_type: llm_gateway::ProviderType::GGUF,
...
async fn distillation_analyze(
    ...
    let model_path = workspace_sovereign_model_path(&sovereign);
    ...
    match result {
        Ok(Ok((analysis, _sov))) => {
            let genome = crate::federation::graph::analysis_to_genome_json(&analysis);
            Json(serde_json::json!({
                "ok": true,
                "sovereign": sovereign,
                "model_path": workspace_sovereign_model_path(&sovereign).to_string_lossy(),
                "analysis": {
...
struct DnaDissectRequest {
    /// Path to the GGUF model to dissect (e.g. "/path/to/models/foundation_v1.gguf")
    /// or just a filename to look up in the models directory
    model: String,
...
async fn dna_compare(
    ...
    use adaptation_plane::ProfileAnalyzer;
    ...
    match (dna_a, dna_b) {
        (Some(a), Some(b)) => {
            ...
            use adaptation_plane::{AgentProfile, ProfileLocus};
            ...
            let distance = ProfileAnalyzer::distance(&gn_a, &gn_b);
...
async fn models_registry(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    Json(serde_json::json!({
        ...
        "models_dir":     workspace_models_dir().to_string_lossy(),
        "inbox_dir":      workspace_models_inbox_dir().to_string_lossy(),
        "models":         models,
...
async fn vault_query(
    ...
    let q = VaultQuery {
        model_name: req.model_name.clone().unwrap_or_default(),
        block_range,
...
async fn specialists_export(
    ...
    let models_dir_buf = workspace_models_dir();
    let models_dir = models_dir_buf.as_path();
    let export_dir = workspace_exports_dir();
...
async fn openai_chat_completions(
    ...
    // Inject user identifier into system context if provided
    let user_hint = req.user.as_ref().map(|u| format!("[User: {}]", u));
    let augmented_message = match user_hint {
        Some(hint) => format!("{}\n\n{}", hint, augmented_message),
        None => augmented_message,
    };
    ...
    if req.stream.unwrap_or(false) {
        ...
        let stream_body = async_stream::stream! {
            ...
            while i < words.len() {
                ...
                yield Ok::<_, std::convert::Infallible>(axum::response::sse::Event::default()
                    .data(serde_json::to_string(&serde_json::json!({
                        "id": id_clone, "object": "text_completion.chunk", "created": created,
                        "model": model_clone,
                        "choices": [{"text": s, "index": 0, "finish_reason": null}]
                    })).unwrap_or_default()));
                i = end;
                tokio::time::sleep(tokio::time::Duration::from_millis(20)).await;
            }
...
async fn route_to_sovereign(
        ...
        && let Some(ref llm) = specialist.llm
    {
        ...
        if (temperature - 0.7).abs() > 0.01 || max_tokens != 2048 {
            ...
            let scoped_llm = match llm_gateway::LLMClient::new(config).await {
                Ok(c) => c,
    ...
    loop {
        tokio::time::sleep(tokio::time::Duration::from_millis(50)).await;
        {
            ...
            if let Some(r) = new_results.iter().find(|r| {
                r.specialist_name.as_deref() == Some(sovereign_name_owned.as_str())
