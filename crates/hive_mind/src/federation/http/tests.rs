...
mod tests {
    ...
    use hypervisor::rate_limit::{TokenBucketConfig, TokenBucketLimiter};
    ...
    fn fresh_federation_with_all() -> Arc<Federation> {
        Arc::new(
            Federation::builder(
                hypervisor::persistence::PersistenceManager::new(":memory:").unwrap(),
            )
            .with_all()
            .build(),
        )
    ...
    async fn test_rate_limit_per_route_uses_independent_buckets() {
        ...
        assert_eq!(
            chat_limiter.tracked_key_count(),
            1,
            "chat limiter saw 1 key"
        );
        assert_eq!(
            state.rate_limiter.tracked_key_count(),
            1,
            "default limiter saw 1 key"
        );
    }
