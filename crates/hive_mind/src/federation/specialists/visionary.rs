...
use llm_gateway::types::DesignContext;
...
impl LearningData {
    ...
    /// Record design feedback and update improvement score
    pub fn record_feedback(&mut self, feedback: &DesignFeedback) {
        self.feedback_history.push(feedback.clone());

        let approval_count = self.feedback_history.iter().filter(|f| f.approved).count();
        let total = self.feedback_history.len();
        self.confidence_score = if total > 0 {
            approval_count as f32 / total as f32
        } else {
            0.5
        };
    }
...
impl Visionary {
    ...
    fn learn_from_feedback(&self, feedback: &DesignFeedback) {
        let mut learning = self.learning.lock();
    ...
    async fn execute(&self, decision: &Decision) -> Result<ExecutionResult, SpecialistError> {
        ...
        let (output, success) = if let Some(llm) = &self.llm {
            ...
            match llm.generate_design(&ctx).await {
                Ok(generation) => {
                    let output = format!("LLM generated design for '{}': {}", intent, generation);
                    (output, true)
                }
                Err(e) => {
...
mod tests {
    ...
    fn test_generate_variants() {
        let visionary = Visionary::new();
        let variants = visionary.generate_variants_for(5, "");
    ...
    fn test_learn_from_feedback() {
        ...
        assert_eq!(visionary.learning.lock().feedback_history.len(), 1);
    }
    ...
    async fn test_llm_design_generation_directly() {
        ...
        assert!(!generation.variants.is_empty());
    }
