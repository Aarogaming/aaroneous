...
pub struct Phygital {
    ...
    pub ar_provider: Option<Arc<dyn ArProvider>>,
}
...
impl Phygital {
    ...
    pub fn save_learning_to(
        &self,
        pm: &hypervisor::persistence::PersistenceManager,
    ...
    pub async fn with_ar(mut self) -> Result<Self, ArError> {
        ...
        impl crate::federation::ar::ArProvider for DefaultArProvider {
            ...
            fn get_system_info(&self) -> crate::federation::ar::ArSystemInfo {
                crate::federation::ar::ArSystemInfo {
                    runtime_name: "stub".to_string(),
                    runtime_version: "0.0".to_string(),
                    system_name: "Stub".to_string(),
                    vendor_id: 0,
                    form_factor: crate::federation::ar::FormFactor::HeadMountedDisplay,
                    view_configuration: crate::federation::ar::ViewConfiguration::Mono,
                    tracks_position: true,
                    supports_passthrough: false,
                }
            }
        }
        self.ar_provider = Some(Arc::new(DefaultArProvider));
        Ok(self)
    }

    /// Attach an already-spawned AR provider
    pub fn attach_ar(&mut self, provider: Arc<dyn ArProvider>) {
    ...
    pub fn detect_ar_hardware_real(&mut self) -> Result<Option<SpatialDevice>, ArError> {
        let provider: &Arc<dyn ArProvider> = match &self.ar_provider {
            Some(provider) => provider,
            None => return Err(ArError::FeatureNotEnabled),
        };
    ...
    pub async fn begin_ar_session(&self) -> Result<(), ArError> {
        ...
        let provider: &dyn crate::federation::ar::ArProvider = &**provider;
