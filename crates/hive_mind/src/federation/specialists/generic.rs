...
pub struct GenericSpecialist {
    ...
    pub persona: Option<adaptation_plane::SpecialistPersona>,
}
...
impl GenericSpecialist {
    ...
    pub async fn with_gguf_path(mut self, path: impl Into<std::path::PathBuf>) -> Self {
        ...
        if persona_path.exists()
            ...
            && let Ok(persona) = serde_json::from_str::<adaptation_plane::SpecialistPersona>(&data)
        {
            info!(
    ...
    ///
    /// Saves to the workspace data directory so the sovereign's
    ...
    pub fn save_memory_to_disk(&self) -> anyhow::Result<()> {
        ...
        let path = hypervisor::workspace::WorkspacePaths::workspace_root()
            .join("data")
            .join("memories")
                        ...
                        && let Some(end) = response[start..].find("[/WRITE_FILE]")
                    {
                            ...
                            && let Some(path_end) = block[path_start..].find('\n')
                        {
                            ...
                            if let Some(content_start) = block.find("content:") {
                                ...
                                let abs_path =
                                    hypervisor::workspace::WorkspacePaths::workspace_root()
                                        .join(file_path);
    ...
    let role = match domain {
        "code_review" | "code" | "coding" | "code_generation" => {
            "You are an expert software engineer. Review code, identify bugs, \
             ...
             You have Self-Coding capabilities. If you need to autonomously write or edit code in the workspace (AARONEOUS_WORKSPACE), \
