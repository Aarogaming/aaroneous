...
impl LiveP2PDaemon {
    pub fn current_lineage() -> String {
        let pkg = env!("CARGO_PKG_VERSION");
        if let Some(idx) = pkg.find("+vb.") {
            pkg[idx + 4..].to_string()
        } else {
            "unknown".to_string()
        }
    ...
    async fn handle_incoming_stream(&self, stream: TcpStream, addr: SocketAddr) -> Result<()> {
        ...
        let _ = tx
            .send(DaemonWirePacket::Ping {
                ...
                send_ts_ms: now_ms,
                lineage_stamp: Self::current_lineage(),
            })
            .await;
                            ...
                            && let Some(id) = sender_id
                        {
                            ...
                            self.peers.write().insert(
                                ...
                                LivePeerInfo {
                                    ...
                                    latency_ms: 1.0,
                                    lineage_stamp: "unknown".to_string(),
                                    is_connected: true,
                                    messages_sent: 1,
                                    messages_received: 1,
                                    last_seen_ms: now_ms,
    ...
    async fn process_packet(
        ...
        match packet {
            DaemonWirePacket::Ping {
                ...
                if lineage_stamp != Self::current_lineage() {
                    warn!(
                        target: "federation::swarm",
                        peer = %from_node,
                        peer_lineage = %lineage_stamp,
                        my_lineage = %Self::current_lineage(),
                        "?? Peer Lineage Mismatch! P2P offloading may be non-deterministic."
                    );
                }
            ...
            DaemonWirePacket::Pong {
                ...
                if let Some(peer) = self.peers.write().get_mut(&from_node) {
                    ...
                    peer.lineage_stamp = lineage_stamp;
                }
                Ok(Some(from_node))
            }
    ...
    pub fn select_best_peer(&self) -> Result<String> {
        ...
        let my_lineage = Self::current_lineage();
        let mut candidates: Vec<_> = peers
            .values()
            .filter(|p| {
                p.is_connected && channels.contains_key(&p.peer_id) && p.lineage_stamp == my_lineage
            })
            .collect();
        if candidates.is_empty() {
            bail!(
                "No connected peer hives with matching lineage available for swarm task offloading"
            );
        }
