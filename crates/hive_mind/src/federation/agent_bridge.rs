...
use orchestrator::{Domain, SpecialistAgent};
...
impl SpecialistAgentBridge {
    ...
    /// use hive_mind::federation::agent_bridge::SpecialistAgentBridge;
    /// use hive_mind::federation::specialists::Visionary;
    /// use hive_mind::federation::learn_persist::PersistableLearning;
    /// use orchestrator::create_specialist;
    ///
...
mod tests {
    ...
    async fn test_bridge_execute() {
        let agent = create_specialist("synthesizer").unwrap();
        let bridge = SpecialistAgentBridge::new(agent).unwrap();
