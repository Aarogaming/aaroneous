...
use adaptation_plane::{AgentProfile, LociSource, ProfileCategory, ProfileLocus};
...
pub async fn dissect_model(
    ...
    let emit = |stage: DissectionStage, blocks_done: usize, blocks_total: usize, msg: &str| {
        if let Some(ref tx) = progress_tx {
            let pct = (blocks_done * 100).checked_div(blocks_total).unwrap_or(0) as u8;
    ...
    emit(
        DissectionStage::ReadingHeader,
        0,
        0,
        "Memory-mapping model file and reading GGUF header",
    );
    ...
    let tensors: HashMap<String, TensorMeta> = index
        .0
        .into_values()
        .next()
        .map(|gm| gm.tensors)
    ...
    emit(
        DissectionStage::MappingTensors,
        0,
        0,
        &format!(
            "Header read: {} tensors, arch={}",
            tensor_count, architecture
        ),
    );
    ...
    emit(
        ...
        num_blocks,
        &format!(
            "Detected {} transformer blocks, ~{:.0}M params",
            num_blocks, parameter_count_m
        ),
    );
    ...
    for block_idx in 0..num_blocks {
        emit(
            DissectionStage::AnalyzingBlock,
            block_idx,
            num_blocks,
    ...
    emit(
        DissectionStage::ComputingEmbeddings,
        num_blocks,
        num_blocks,
        "Analyzing embedding layer",
    );
    let (emb_density, emb_norm_mean, emb_norm_std) = analyze_embeddings(&tensors, &mmap);

    // Cross-block correlation
    emit(
        DissectionStage::ComputingCorrelations,
        num_blocks,
        num_blocks,
    ...
    emit(
        DissectionStage::BuildingGenome,
        num_blocks,
        num_blocks,
        "Building genome loci",
    );
    let (genome_loci, genetic_loci) = build_genome(&GenomeData {
        blocks: &blocks,
        correlations: &cross_block_correlation,
        emb_density,
        emb_norm_mean,
        emb_norm_std,
    ...
    emit(
        DissectionStage::Persisting,
        num_blocks,
        num_blocks,
        "Saving DNA sidecar JSON",
    );
    ...
    emit(
        DissectionStage::Complete,
        num_blocks,
        num_blocks,
        &format!(
            "Dissection complete in {:.1}s — {} loci extracted",
            duration_secs,
    ...
    info!(
        "DNA dissection complete: {} blocks, {} genetic loci, {:.1}s",
        dna.num_blocks,
        dna.genetic_loci.len(),
        duration_secs
    );
...
fn analyze_embeddings(
    ...
    if vocab_size == 0 || embed_dim == 0 || t.size == 0 {
        return (0.0, 0.0, 0.0);
    }
    ...
    (
        ...
        mean_norm.clamp(0.0, 5.0) / 5.0,
        std_dev.clamp(0.0, 2.0) / 2.0,
    )
}
...
fn compute_cross_block_correlation(blocks: &[BlockDNA]) -> Vec<f64> {
    ...
    for i in 0..blocks.len() - 1 {
        ...
        let av = [
            ...
            a.mlp_weight_mean,
            a.gate_sparsity,
            a.attn_frobenius_norm,
        ];
        let bv = [
            b.attn_weight_mean,
            b.mlp_weight_mean,
            b.gate_sparsity,
            b.attn_frobenius_norm,
        ];
...
pub struct GenomeData<'a> {
    pub blocks: &'a [BlockDNA],
    pub correlations: &'a [f64],
    pub emb_density: f64,
    pub emb_norm_mean: f64,
    pub emb_norm_std: f64,
...
fn build_genome(data: &GenomeData) -> (HashMap<String, f64>, Vec<GeneticLocusRecord>) {
    ...
    let mean_attn: f64 = if data.blocks.is_empty() {
        0.0
    } else {
        data.blocks.iter().map(|b| b.attn_weight_mean).sum::<f64>() / data.blocks.len() as f64
    };
    let mean_mlp: f64 = if data.blocks.is_empty() {
        0.0
    } else {
        data.blocks.iter().map(|b| b.mlp_weight_mean).sum::<f64>() / data.blocks.len() as f64
    };
    let attn_mlp_ratio = if mean_mlp > 0.0 {
        (mean_attn / mean_mlp).clamp(0.0, 2.0)
    } else {
        1.0
    };

    let mean_gate_sparsity: f64 = if data.blocks.is_empty() {
        0.0
    } else {
        data.blocks.iter().map(|b| b.gate_sparsity).sum::<f64>() / data.blocks.len() as f64
    };

    let mean_corr: f64 = if data.correlations.is_empty() {
        1.0
    } else {
        data.correlations.iter().sum::<f64>() / data.correlations.len() as f64
    };
    let corr_variance: f64 = if data.correlations.is_empty() {
        0.0
    } else {
        data.correlations
            .iter()
    ...
    let spec_scores: Vec<f64> = {
        ...
        if max_score > 0.0 {
            attn_scores
                .iter()
                .map(|&s| (s / max_score).clamp(0.0, 1.0))
                .collect()
        } else {
            vec![0.5; data.blocks.len()]
        }
    };

    let layer_spec_variance: f64 = {
        let mean = spec_scores.iter().sum::<f64>() / spec_scores.len().max(1) as f64;
        spec_scores.iter().map(|&s| (s - mean).powi(2)).sum::<f64>()
            / spec_scores.len().max(1) as f64
    };
    ...
    let model_density = (data.file_size_mb / 120_000.0).clamp(0.0, 1.0);
    let param_density = (data.parameter_count_m / 200_000.0).clamp(0.0, 1.0);
    ...
    genome_loci.insert(
        "depth".into(),
        (data.blocks.len() as f64 / 32.0).clamp(0.0, 1.0),
    );
    ...
    genome_loci.insert("embedding_activation_density".into(), data.emb_density);
    genome_loci.insert("embedding_norm_mean".into(), data.emb_norm_mean);
    genome_loci.insert(
        "cross_block_correlation_mean".into(),
        mean_corr.clamp(0.0, 1.0),
    );
    genome_loci.insert(
        "layer_specialization_variance".into(),
        layer_spec_variance.clamp(0.0, 1.0),
    );
    genome_loci.insert("attn_mlp_ratio".into(), attn_mlp_ratio / 2.0);

    // ── Build GeneticLocus records ─────────────────────────────────────────

    let add = |loci: &mut Vec<GeneticLocusRecord>,
               id: &str,
               cat: &str,
               src: &str,
               val: f64,
    ...
    add(
        ...
        0.88,
        &format!(
            "Attention-to-MLP weight ratio {:.3} — {} model",
            attn_mlp_ratio,
            if attn_mlp_ratio > 1.1 {
                "relational/associative"
            } else if attn_mlp_ratio < 0.9 {
                "factual/recall-heavy"
            } else {
                "balanced"
            }
        ),
    ...
    add(
        &mut genetic_loci,
        "gate_sparsity",
        "SpecializationGenetics",
        "WeightAnalysis",
        mean_gate_sparsity,
        0.85,
        &format!(
            "FFN gate sparsity {:.1}% — {} specialization",
            mean_gate_sparsity * 100.0,
            if mean_gate_sparsity > 0.6 {
                "high domain"
            } else if mean_gate_sparsity > 0.3 {
                "moderate domain"
            } else {
                "general purpose"
            }
    ...
    add(
        ...
        0.82,
        &format!(
            "Cross-block correlation variance {:.3} — {} hierarchical abstraction",
            depth_gradient,
            if depth_gradient > 0.4 {
                "strong"
            } else {
                "shallow"
            }
    ...
    add(
        ...
        0.80,
        &format!(
            "Token embedding activation density {:.1}% — {} vocabulary coverage",
            data.emb_density * 100.0,
            if data.emb_density > 0.85 {
                "broad general"
            } else {
                "focused domain"
            }
    ...
    // Per-block loci
    for (i, (&spec, block)) in spec_scores.iter().zip(data.blocks.iter()).enumerate() {
        add(
            &mut genetic_loci,
            &format!("block_{}_attn", i),
            "AttentionGenetics",
            "WeightAnalysis",
            block.attn_weight_mean,
            0.72,
        ...
        add(
            &mut genetic_loci,
            &format!("block_{}_spec", i),
            "SpecializationGenetics",
            "WeightAnalysis",
            spec,
            0.75,
            &format!(
                "Block {} specialization score (deviation from model mean)",
                i
            ),
        );
    }
...
fn dequantize_sample(bytes: &[u8], dtype: u32, n: usize) -> Vec<f32> {
    ...
    match dtype {
        ...
        8 => {
            ...
            let n_blocks = (bytes.len() / block_size).min(n.div_ceil(32));
            let mut out = Vec::with_capacity(n_blocks * 32);
            for b in 0..n_blocks {
                let off = b * block_size;
                if off + block_size > bytes.len() {
                    break;
                }
        ...
        12 => {
            ...
            let n_blocks = (bytes.len() / block_size).min(n.div_ceil(256));
            ...
            for b in 0..n_blocks {
                ...
                for j in 0..32_usize {
                    if out.len() >= n || data_start + j >= bytes.len() {
                        break;
                    }
...
pub fn dna_to_genome(dna: &ModelDNA) -> AgentProfile {
    use adaptation_plane::AdaptationState;
