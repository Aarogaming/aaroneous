...
use hypervisor::workspace::WorkspacePaths;
    ...
    if auto_dissect {
        ...
        match crate::federation::dna::dissect_model(&target_path, Some(tx)).await {
            Ok(dna) => {
                ...
                let engine = adaptation_plane::DigestionEngine::new(
                    adaptation_plane::DigestionConfig::default(),
                    event_tx,
                );
                let task = adaptation_plane::DigestionTask {
