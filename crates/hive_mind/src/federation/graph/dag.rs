...
impl SovereignGraph {
    ...
    pub fn has_path(&self, from: &str, to: &str) -> bool {
        ...
        while let Some(current) = queue.pop_front() {
            ...
            if let Some(idxs) = self.forward.get(&current) {
                for &i in idxs {
                    queue.push_back(self.edges[i].to.clone());
                }
            }
        }
        false
    ...
    pub fn roots(&self) -> Vec<&Node> {
        self.nodes
            .values()
            .filter(|n| self.reverse[&n.id].is_empty())
            .collect()
    }
    ...
    pub fn successors(&self, id: &str) -> Vec<&Node> {
        self.forward
            ...
            .map(|idxs| {
                idxs.iter()
                    .filter_map(|&i| self.nodes.get(&self.edges[i].to))
                    .collect()
            })
            .unwrap_or_default()
    }
    ...
    pub fn predecessors(&self, id: &str) -> Vec<&Node> {
        self.reverse
            ...
            .map(|idxs| {
                idxs.iter()
                    .filter_map(|&i| self.nodes.get(&self.edges[i].from))
                    .collect()
            })
            .unwrap_or_default()
    }
    ...
    pub fn topological_order(&self) -> Result<Vec<&Node>, GraphError> {
        let mut in_degree: HashMap<&str, usize> = self
            .nodes
            .keys()
            .map(|id| (id.as_str(), self.reverse[id].len()))
            .collect();
        ...
        if result.len() != self.nodes.len() {
            Err(GraphError::CycleDetected("?".into(), "?".into()))
        } else {
            Ok(result)
        }
    }
    ...
    pub fn parallel_layers(&self) -> Vec<Vec<&Node>> {
        ...
        while let Some((id, d)) = queue.pop_front() {
            ...
            if let Some(idxs) = self.forward.get(id) {
                for &i in idxs {
                    queue.push_back((self.edges[i].to.as_str(), d + 1));
                }
            }
        }
    ...
    pub fn bfs(&self, start: &str, max_depth: usize) -> Vec<(usize, &Node)> {
        ...
        while let Some((id, depth)) = queue.pop_front() {
            ...
            if let Some(node) = self.nodes.get(&id) {
                ...
                if let Some(idxs) = self.forward.get(&id) {
                    for &i in idxs {
                        queue.push_back((self.edges[i].to.clone(), depth + 1));
                    }
                }
            }
        }
        ...
        if let Some(deps) = task.get("deps").and_then(|d| d.as_array()) {
            for dep in deps {
                if let Some(dep_id) = dep.as_str() {
                    graph
                        .add_edge(Edge::new(dep_id, id, EdgeKind::DependsOn))
                        .ok();
                }
            }
        }
...
pub fn model_lineage_graph(registry_json: &serde_json::Value) -> SovereignGraph {
    let mut graph = SovereignGraph::new();
    if let Some(models) = registry_json
    ...
    let models_dir = hypervisor::workspace::WorkspacePaths::workspace_root().join("models");
    for (id, label, filename) in &sovereigns {
        let full_path = models_dir.join(filename).to_string_lossy().to_string();
        let size = std::path::Path::new(&full_path)
            .metadata()
            .map(|m| m.len())
            .unwrap_or(0);
        let node = Node::new(
        ...
        .with_tag("sovereign")
        .with_tag("crystallized");
        graph.add_node(node).ok();
        graph
            .add_edge(Edge::new(
                "foundation_v1",
                *id,
                EdgeKind::CrystallizedFrom {
                    blocks_taken: vec![],
                    tensor_count: 0,
                },
            ))
...
mod tests {
    ...
    fn test_dag_topology() {
        ...
        for id in &["a", "b", "c"] {
            g.add_node(make_custom(id)).unwrap();
        }
    ...
    fn test_cycle_detection() {
        ...
        g.add_edge(Edge::new("x", "y", EdgeKind::DependsOn))
            .unwrap();
        g.add_edge(Edge::new("y", "z", EdgeKind::DependsOn))
            .unwrap();
        assert!(matches!(
            g.add_edge(Edge::new("z", "x", EdgeKind::DependsOn)),
    ...
    fn test_parallel_layers() {
        ...
        g.add_edge(Edge::new("a", "c", EdgeKind::DependsOn))
            .unwrap();
        g.add_edge(Edge::new("b", "c", EdgeKind::DependsOn))
            .unwrap();
