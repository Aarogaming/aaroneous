...
impl RaftNode {
    ...
    pub fn become_follower(&self, term: Term, leader: Option<NodeId>) -> Result<(), String> {
        ...
        let mut state = self.state.write().map_err(|_| "Failed to acquire lock")?;
    ...
    pub fn become_leader(&self) -> Result<(), String> {
        ...
        let leader_state = LeaderState::new(self.config.all_nodes.clone(), last_log_index);
    ...
    pub fn handle_append_entries(
        ...
        let mut persistent = self
            ...
            .write()
            .map_err(|_| "Failed to acquire lock")?;
        let volatile = self.volatile.read().map_err(|_| "Failed to acquire lock")?;
        ...
        let prev_index_in_vec =
            (rpc.prev_log_index as usize).saturating_sub(persistent.last_included_index as usize);
        ...
        if !rpc.entries.is_empty() {
            ...
            let remove_from =
                (first_new_index as usize).saturating_sub(persistent.last_included_index as usize);
        ...
        for entry in &rpc.entries {
            ...
            let expected_index =
                persistent.last_included_index + persistent.log_entries.len() as LogIndex + 1;
        ...
        if rpc.leader_commit > volatile.commit_index {
            let last_log_index =
                persistent.last_included_index + persistent.log_entries.len() as LogIndex;
