...
impl Snapshot {
    ...
    pub fn compaction_ratio(&self, total_log_entries: usize) -> f64 {
        ...
        self.last_included_index as f64
            / (self.last_included_index as f64 + total_log_entries as f64)
    }
}
...
mod tests {
    ...
    fn test_snapshot_store_get_before() {
        ...
        assert_eq!(
            store.get_snapshot_before(25).unwrap().last_included_index,
            30
        );

        // Index 30 is covered by snap3
        assert_eq!(
            store.get_snapshot_before(30).unwrap().last_included_index,
            30
        );

        // Index 5 is covered by snap1, snap2, snap3 - but most recent (snap3) is returned
        assert_eq!(
            store.get_snapshot_before(5).unwrap().last_included_index,
            30
        );
        ...
        assert_eq!(
            store.get_snapshot_before(10).unwrap().last_included_index,
            30
        );
    }
