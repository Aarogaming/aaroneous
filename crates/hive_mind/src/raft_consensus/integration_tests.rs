...
mod tests {
    ...
    fn test_log_replication_single_entry() {
        ...
        let result = engine
            .replicate_log_entry(&"node1".to_string(), 0, 0, vec![entry], 1)
            .unwrap();
    ...
    fn test_log_replication_multiple_entries() {
        ...
        let result = engine
            .replicate_log_entry(&"node1".to_string(), 0, 0, entries, 3)
            .unwrap();
    ...
    fn test_multi_node_append_entries() {
        ...
        let entries = vec![create_entry(1, 1, "op1"), create_entry(2, 1, "op2")];

        let result = engine
            .replicate_log_entry(&"node1".to_string(), 0, 0, entries, 2)
            .unwrap();
    ...
    fn test_cluster_consistency_multiple_operations() {
        ...
        for i in 1..=5 {
            ...
            let result = engine
                .replicate_log_entry(&"node1".to_string(), i - 1, 1, vec![entry], i)
                .unwrap();
