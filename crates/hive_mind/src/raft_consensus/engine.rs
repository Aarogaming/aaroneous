...
impl RaftEngine {
    ...
    pub fn new(node_ids: Vec<NodeId>) -> Self {
        ...
        let config = RaftConfig::new(node_ids[0].clone(), node_ids.clone());
...
mod tests {
    ...
    fn test_replicate_empty_entries() {
        ...
        let result = engine
            .replicate_log_entry(&"node1".to_string(), 0, 0, vec![], 0)
            .unwrap();
    ...
    fn test_decrement_next_index() {
        ...
        let initial_next = engine
            .get_next_index(&"node1".to_string(), &"node2".to_string())
            .unwrap();
        assert_eq!(initial_next, 3); // 2 entries + 1

        engine
            .decrement_next_index(&"node1".to_string(), &"node2".to_string())
            .unwrap();

        let after_decr = engine
            .get_next_index(&"node1".to_string(), &"node2".to_string())
            .unwrap();
    ...
    fn test_update_match_index() {
        ...
        engine
            .update_match_index(&"node1".to_string(), &"node2".to_string(), 5)
            .unwrap();
