...
use hypervisor::concept_drift::ConceptDriftDetector;
use hypervisor::enzyme_runner::EnzymeRunner;
use hypervisor::hox_registry::HoxRegistry;
use hypervisor::nlm_sentinel::{IntentTier, NlmSentinel};
use hypervisor::predictive_models::{HiddenMarkovModel, KalmanFilter1D};
use hypervisor::semantic_indexing::SemanticIndex;
use hypervisor::specialist_memory::{
    MemoryEntry, MemoryType, SharedMemoryRegistry, SpecialistMemoryStore,
};
use hypervisor::system_metrics::{SystemMetricsCollector, ThermalStatus};
...
pub struct SupervisoryDaemon {
    ...
    pub state_publisher: Arc<hypervisor::state_snapshot::EngineStatePublisher>,
...
impl SupervisoryDaemon {
    ...
    pub fn new_with_pacing(
        ...
        if let Some(ref db_mutex) = hive_db {
            ...
            if let Ok(embeddings) = db.load_all_embeddings() {
                for (id, text, vector, metadata, access_count) in embeddings {
                    let entry = hypervisor::semantic_indexing::SemanticEmbedding {
                        id,
                        text,
                        vector,
                        metadata,
        ...
        Ok(Self {
            ...
            state_publisher: Arc::new(hypervisor::state_snapshot::EngineStatePublisher::new()),
            pacing_regulator,
