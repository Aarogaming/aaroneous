...
pub struct IntentAnalyzer {
    ...
    llm: Option<Arc<llm_gateway::LLMClient>>,
}
...
impl IntentAnalyzer {
    ...
    pub fn with_llm(mut self, client: Arc<llm_gateway::LLMClient>) -> Self {
    ...
    pub fn analyze_observation(&self, observation: &Observation) -> InferredIntent {
        match &observation.player_action {
            ...
            PlayerAction::Block => InferredIntent {
                ...
                confidence: 0.85,
                alternatives: vec![("Create opening for counterattack".to_string(), 0.15)],
                sample_count: 1,
                intent_category: IntentCategory::Defensive,
            },
...
"#,
        ...
        let context = llm_gateway::types::TaskAnalysisContext {
        ...
        match llm.analyze_context(&context).await {
            Ok(analysis) => {
                // Map the LLM's analysis back to InferredIntent
                let category = if analysis
