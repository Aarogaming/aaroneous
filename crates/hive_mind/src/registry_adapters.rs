...
use hypervisor::registry::{EntityInfo, EntryHealth, RegistryType, SubRegistry, WorkspaceContext};
...
> {
    inner: hypervisor::unified_registry::Registry<T>,
}

impl<T: Clone + serde::Serialize + serde::de::DeserializeOwned + Send + Sync>
    UnifiedRegistryAdapter<T>
{
    pub fn new(registry: hypervisor::unified_registry::Registry<T>) -> Self {
        Self { inner: registry }
    }
}

fn map_health(h: hypervisor::unified_registry::EntryHealth) -> EntryHealth {
    match h {
        hypervisor::unified_registry::EntryHealth::Healthy => EntryHealth::Healthy,
        hypervisor::unified_registry::EntryHealth::Degraded => EntryHealth::Degraded,
        hypervisor::unified_registry::EntryHealth::Failed => EntryHealth::Failed,
        hypervisor::unified_registry::EntryHealth::Unknown => EntryHealth::Unknown,
    }
}
...
pub struct LLMModelRegistryAdapter {
    _inner: llm_gateway::model_registry::ModelRegistry,
}

impl LLMModelRegistryAdapter {
    pub fn new(registry: llm_gateway::model_registry::ModelRegistry) -> Self {
        Self { _inner: registry }
    }
}
...
pub struct ChromosomeRegistryAdapter {
    _inner: hypervisor::chromosome_registry::ChromosomeRegistry,
}

impl ChromosomeRegistryAdapter {
    pub fn new(registry: hypervisor::chromosome_registry::ChromosomeRegistry) -> Self {
        Self { _inner: registry }
    }
}
...
pub struct HoxCapabilityRegistryAdapter {
    inner: hypervisor::hox_registry::HoxRegistry,
}

impl HoxCapabilityRegistryAdapter {
    pub fn new(registry: hypervisor::hox_registry::HoxRegistry) -> Self {
        Self { inner: registry }
    }
}
