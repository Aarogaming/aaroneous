...
pub fn retain_ui(enable: bool) -> Result<()> {
    if enable {
        tracing::info!("[fascia] UI routing retained via broker proxy.");
    } else {
        tracing::info!("[fascia] UI routing disabled.");
    }
    Ok(())
}
...
mod tests {
    ...
    #[test]
    fn test_retain_ui() {
        assert!(retain_ui(true).is_ok());
        assert!(retain_ui(false).is_ok());
    }
}
