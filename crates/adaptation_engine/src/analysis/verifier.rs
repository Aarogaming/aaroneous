...
use super::experiment::{ExperimentResult, TestOutcome};
...
impl VerificationResult {
    ...
    pub fn verify(hypothesis: &Hypothesis, result: &ExperimentResult) -> Self {
        ...
        let constellation_update =
            Self::create_constellation_update(&hypothesis.target, result, posterior, &verdict);
    ...
    fn calculate_likelihood(result: &ExperimentResult) -> f64 {
        match &result.outcome {
            TestOutcome::Pass => {
                ...
                let perf_factor = if result.performance_metrics.mean_time_ms < 100.0 {
                    1.0
                } else if result.performance_metrics.mean_time_ms < 500.0 {
                    0.8
                } else {
                    0.6
                };
    ...
    fn generate_evidence_summary(result: &ExperimentResult, posterior: f64) -> String {
        ...
        format!(
            ...
            outcome_str, posterior, result.execution_time_ms,
        )
    }
...
mod tests {
    use super::*;
    use crate::analysis::experiment::{ExperimentResult, PerformanceMetrics};
    use crate::analysis::hypothesis::{ExperimentDesign, Hypothesis, TestType};
    ...
    fn test_verification_fail() {
        ...
        assert!(matches!(
            ...
            Verdict::Falsified | Verdict::Unstable
        ));
        assert!(verification.posterior_confidence < hypothesis.prior_confidence);
    }
