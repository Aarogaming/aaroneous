...
fn run_health_checks() -> bool {
    let persistence_ok = hypervisor::persistence::PersistenceManager::new(":memory:").is_ok();
    if !persistence_ok {
        tracing::error!("Health check: persistence initialization failed");
    }

    let paths = paths::WorkspacePaths::from_config(paths::WorkspacePathsConfig::default());
    if !paths.registry().exists() {
        tracing::warn!("Health check: registry path is absent and will be initialized on demand");
    }
