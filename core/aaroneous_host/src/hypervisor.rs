...
use hive_mind::splicing_engine::WasmSplicingEngine;
use hive_mind::supervisory_loop::SupervisoryDaemon;
use hive_mind::unified_learning::{UnifiedLearningConfig, UnifiedLearningLoop};
...
fn main() -> Result<()> {
    let (_init, _guard) = hypervisor::logging::init_logging();
...
fn run_cli(cli: Cli) -> Result<()> {
    match &cli.command {
        ...
        Some(Commands::Inject { intent }) => {
            use hive_mind::supervisory_loop::{LegacySharedMemorySynapse, SynapseState};
...
fn run_export_capabilities(output: &std::path::Path) -> Result<()> {
    let broker = hive_mind::capability_broker::CapabilityBroker::new();
...
async fn run_mesh_pipeline(nodes_count: usize, live: bool) -> Result<()> {
    ...
    if live {
        ...
        for i in 0..nodes_count {
            ...
            let config = hive_mind::federation::multi_hive::live_daemon::LiveP2PConfig {
            ...
            let daemon = hive_mind::federation::multi_hive::live_daemon::LiveP2PDaemon::new(config);
            daemon.start().await?;
        ...
        let mut offloader = hive_mind::federation::multi_hive::swarm_offloader::SwarmOffloader::new(
            Arc::new(daemons[1].clone()),
            80.0,
        );
        offloader.update_pressure(92.5); // High local pressure triggers remote offload

        let task = hive_mind::federation::multi_hive::swarm_offloader::SwarmTask {
        ...
        match outcome {
            hive_mind::federation::multi_hive::swarm_offloader::SwarmExecutionOutcome::OffloadedToPeer { peer_node_id, duration_us, result_payload } => {
            ...
            hive_mind::federation::multi_hive::swarm_offloader::SwarmExecutionOutcome::ExecutedLocally { duration_us, .. } => {
    ...
    let mut cluster = hive_mind::federation::multi_hive::hive_cluster::HiveCluster::new(
        hive_mind::federation::multi_hive::hive_cluster::ClusterConfig::default(),
    );

    // Bootstrap local primary hive node
    let local_node = hive_mind::federation::multi_hive::hive_cluster::HiveNode::new(
    ...
    for i in 1..nodes_count {
        let peer = hive_mind::federation::multi_hive::hive_cluster::HiveNode::new(
    ...
    let mut gossip = hive_mind::federation::multi_hive::consensus::GossipMessage::new(
...
async fn run_mcp_pipeline(host: &str, port: u16) -> Result<()> {
    ...
    let config = hive_mind::mcp_service::ServiceConfig {
        http_addr: addr,
        ..Default::default()
    };

    let service = Arc::new(hive_mind::mcp_service::McpService::new(config));
    service.register_sovereign_tools().await;
    let mcp_cfg = hive_mind::mcp_service::http_api::McpServiceConfig::default();
    let server = hive_mind::mcp_service::http_api::HttpServer::new(addr, mcp_cfg);
