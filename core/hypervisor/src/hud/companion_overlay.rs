...
impl CompanionTelemetryOverlay {
    ...
    pub fn render(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            ...
            ui.add_space(4.0);
            ui.label(
                egui::RichText::new(&self.last_intercom_reply)
                    .italics()
            ...
            if self.is_expanded {
                ...
                ui.group(|ui| {
                            ...
                            && !self.chat_input_buffer.trim().is_empty()
                        {
                            ...
                            if let Some(bridge) = &self.transformer_bridge {
                                bridge.dispatch_command(
                                    crate::hud::FrontendCommandSignal::TransduceLinguisticIntent {
                                        prompt: query.clone(),
                                    },
                                );
                            }

                            self.last_intercom_reply =
                                format!("Intent transducted: [{}]. Opcode graph executed.", query);
                    ...
                    ui.label(
                        egui::RichText::new(
                            "Drag & Drop `.si` or `.si-pack` cartridge here to mount",
                        )
                        .weak(),
                    );
                });
