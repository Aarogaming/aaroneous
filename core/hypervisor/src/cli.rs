...
fn default_db_path() -> String {
    WorkspacePaths::discover().hive_db().to_string_lossy().to_string()
}

fn default_inbox_path() -> String {
    WorkspacePaths::discover().inbox().to_string_lossy().to_string()
}
...
pub enum Commands {
    ...
    Start {
        ...
        /// Inbox folder path
        #[arg(long, default_value_t = default_inbox_path())]
        inbox: String,
    },
...
pub enum StatusCmd {
    ...
    /// Inspect or refresh the SAB matrix cache
    SabMatrix {
        /// Force rebuild from sab_*.json manifests
        #[arg(long)]
        refresh: bool,
    },
}
...
pub async fn execute(args: CliArgs) -> Result<(), Box<dyn std::error::Error>> {
    ...
    match args.command {
        ...
        Commands::Dashboard => {
            println!("Launching Aaroneous Graphical Dashboard...");
...
fn execute_install_service(
    ...
    #[cfg(not(windows))]
    {
        ...
        println!("Example systemd unit at: {}/registry/aaroneous.service", WorkspacePaths::discover().root().display());
    }
...
async fn execute_start(
    ...
    let gguf_search = [
        ...
        paths.sovereign_model("ariel"),
        // Foundation model fallback
        paths.models().join("foundation_v1.gguf"),
        // Legacy abliterated variants
        paths.models().join("qwen2.5-1.5b-instruct-abliterated.gguf"),
        paths.models().join("qwen2.5-1.5b.gguf"),
    ...
    let visionary = if let Some(gguf_path) = found_gguf {
        ...
        info!("  To enable real inference: place a Qwen2.5 .gguf in {}", paths.models().display());
    ...
    let registry_path = paths.config().join("specialist_registry.json");
    ...
    // --- REST API Server for Maelstrom UI (port 8765) ---
    let rest_api = if dashboard != "none" {
        let _addr: std::net::SocketAddr = dashboard
            .parse()
            .unwrap_or_else(|_| "127.0.0.1:8765".parse().unwrap());

        use crate::federation::http::RestApiServer;
        match RestApiServer::new(fed.clone()) {
            Ok(srv) => {
                let local_addr = srv.local_addr();
                println!(
                    "Maelstrom REST API server: http://{}",
                    local_addr
                );
                println!("  UI Dashboard:     http://127.0.0.1:1420");
                println!("    Sessions:       POST /sessions, GET /sessions");
                println!("    Intent:         POST /sessions/:id/intent");
                println!("    Results Stream: SSE /sessions/:id/results/stream");
                println!("    Specialists:    GET /specialists");
                println!("    Models:         GET /models/external, POST /models/import");
                println!("    Forge:          POST /forge/crystallize-roster");
                println!("    Scheduler:      CRUD /scheduler/tasks");
                Some(srv)
            }
            Err(e) => {
                tracing::warn!("Could not start REST API: {}", e);
                None
            }
        }
    } else {
    ...
    let local_addr = rest_api.as_ref().map(|s| s.local_addr());
    ...
    println!("Sentinel is arbitrating proposals every 100ms.");
...
async fn execute_status(
    ...
    match cmd {
        ...
        StatusCmd::Metrics { perf: _, resources } => {
        ...
        StatusCmd::SabMatrix { refresh } => {
            use crate::SabMatrix;

            let matrix = if refresh {
                SabMatrix::refresh_generated_cache()?
            } else {
                SabMatrix::load_generated()?
            };

            let count = matrix.surfaces.len();
            println!("SAB Matrix");
            println!("  Surfaces: {}", count);
            println!("  Cache:    registry/sab_matrix.generated.json");
            println!("  Mode:     {}", if refresh { "refreshed" } else { "loaded" });
        }
    }
...
async fn execute_config(cmd: ConfigCmd) -> Result<(), Box<dyn std::error::Error>> {
    match cmd {
        ConfigCmd::Show { all: _, json } => {
            info!("Showing configuration");
            let db_path_display = WorkspacePaths::discover().hive_db();
            if json {
                println!("{{ \"db_path\": \"{}\", \"log_level\": \"info\" }}", db_path_display.display().to_string().replace('\\', "\\\\"));
            } else {
                println!("⚙️  Current Configuration");
                println!("   DB Path: {}", db_path_display.display());
...
mod tests {
    ...
    #[test]
    fn test_cli_parsing_sab_matrix_refresh() {
        let args = vec!["aaroneous", "status", "sab-matrix", "--refresh"];
        let parsed = CliArgs::try_parse_from(args);
        assert!(parsed.is_ok());
    }
