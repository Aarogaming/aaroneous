...
use crate::mcp_service::{ServiceConfig, CapabilityDomain};
...
impl McpService {
    ...
    pub async fn register_sovereign_tools(&self) {
        ...
        tools.push(McpTool::new("federated_task_dispatch",
        ...
        tools.push(McpTool::new("metabolic_heartbeat",
    ...
    async fn handle_tool_call(
        ...
        if tool_name == "register_shard" {
            if let Some(ref fed) = self.federation {
                let shard_name = args.get("name").and_then(|v| v.as_str()).unwrap_or("unknown_shard");
                let capabilities = args.get("capabilities").and_then(|v| v.as_array())
                    .map(|arr| arr.iter().filter_map(|v| v.as_str().map(|s| s.to_string())).collect::<Vec<_>>())
                    .unwrap_or_default();
                let endpoint = args.get("endpoint").and_then(|v| v.as_str()).unwrap_or("");
                
                info!("Internalizing AAS Shard: {} (capabilities={:?}, endpoint={})", shard_name, capabilities, endpoint);
                ...
                if !dynamic.iter().any(|s| s.name == shard_name) {
                    ...
                    let domain = capabilities.first().cloned().unwrap_or_else(|| "general".to_string());
                ...
                return JsonRpcResponse::ok(id, serde_json::json!({
                    "status": "success",
    ...
    async fn tool_register_shard(&self, args: &serde_json::Value) -> anyhow::Result<String> {
        ...
        let endpoint = args.get("endpoint").and_then(|v| v.as_str()).unwrap_or("");
        let session_id = args.get("_meta").and_then(|m| m.get("session_id")).and_then(|s| s.as_str()).map(|s| s.to_string());
        ...
        if let Some(ref fed) = self.federation {
            ...
            if dynamic.iter().any(|s| s.name == name) {
                return Ok(format!("Shard '{}' is already registered and active.", name));
            }
            ...
            Ok(format!("Successfully internalized AAS Shard '{}'. You are now a first-class specialist in the Aaroneous Hive.", name))
        } else {
    ...
    async fn tool_metabolic_heartbeat(&self, args: &serde_json::Value) -> anyhow::Result<String> {
        ...
        if let Some(ref fed) = self.federation {
            ...
            let token_req = args.get("token_request").and_then(|v| v.as_f64()).unwrap_or(1.0) as f32;
    ...
    async fn tool_signal_wasms(&self, args: &serde_json::Value) -> anyhow::Result<String> {
        let signal_type = args.get("signal_type").and_then(|v| v.as_str())
        ...
        if let Some(ref fed) = self.federation {
            ...
            Ok(format!("Signal '{}' broadcasted to WASM runtime via E-Bus intent.", signal_type))
        } else {
            Err(anyhow::anyhow!("Federation not attached"))
        }
    }
    ...
    async fn tool_memory_sync(&self, args: &serde_json::Value) -> anyhow::Result<String> {
        let shard_name = args.get("shard_name").and_then(|v| v.as_str())
            ...
            match action {
                "push" => {
                    ...
                    for entry_json in entries {
                        ...
                        if let Ok(entry) = serde_json::from_value::<crate::specialist_memory::MemoryEntry>(entry_json.clone()) {
                    ...
                    Ok(format!("Successfully synced {} memory entries for shard '{}'.", count, shard_name))
                },
                "pull" | "list" => {
                    let dynamic = fed.dynamic.read().await;
                    if let Some(spec) = dynamic.iter().find(|s| s.name == shard_name) {
                        ...
                        Err(anyhow::anyhow!("Shard '{}' not found in federation", shard_name))
                    }
                },
    ...
    /// Dispatch a task to the federated guild.
    async fn tool_federated_task_dispatch(&self, args: &serde_json::Value) -> anyhow::Result<String> {
        ...
        if let Some(ref fed) = self.federation {
            ...
            intent.context.insert("task_id".to_string(), task_id.to_string());
            intent.context.insert("source".to_string(), "Guild_Federation".to_string());
            
            fed.submit_intent(intent).await;
            
            Ok(format!("Task '{}' dispatched to Aaroneous Core via Federated Intent bridge.", task_id))
        } else {
            Err(anyhow::anyhow!("Federation not attached"))
        }
    }
