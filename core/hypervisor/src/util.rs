...
pub fn lock_write<T>(lock: &RwLock<T>) -> Result<RwLockWriteGuard<'_, T>, HypervisorError> {
    let err_msg = "Failed to acquire write lock";
    match std::str::from_utf8(err_msg.as_bytes()) {
        Ok(s) => lock
            .write()
            .map_err(|_e| HypervisorError::RuntimeError(s.to_string())),
        Err(_) => lock
            .write()
            .map_err(|_e| HypervisorError::RuntimeError(err_msg.to_string())),
    }
}

/// Acquire a read lock on an `RwLock`, mapping any poisoning error to `HypervisorError`.
pub fn lock_read<T>(lock: &RwLock<T>) -> Result<RwLockReadGuard<'_, T>, HypervisorError> {
...
pub fn mutex_lock<T>(m: &Mutex<T>) -> Result<MutexGuard<'_, T>, HypervisorError> {
