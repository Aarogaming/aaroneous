...
impl AutoFabricator {
    ...
    pub async fn fabricate(&self, crate_name: &str) -> Result<PathBuf> {
        let work_dir = PathBuf::from("fabrication").join(crate_name);
        if work_dir.exists() {
            fs::remove_dir_all(&work_dir)?;
        }

        fs::create_dir_all(PathBuf::from("sabs"))?;

        // 1. Copy template
        self.copy_dir(&PathBuf::from("sab_template"), &work_dir)?;
        ...
        let prompt = format!(
            ...
            2. Wrap basic features of {} and return the result as a CString pointer",
        ...
        let mut dep_str = self
            .extract_section(&response, "[DEPENDENCY]", "[/DEPENDENCY]")
        ...
        if let Some(rust_code) = rust_str
            && rust_code.contains("execute_task")
        {
        ...
        if !build_cmd.status.success() {
            ...
            tracing::warn!(
                "AutoFabricator: WASM compilation failed for {}. Attempting native fallback. Error snippet: {}",
                crate_name,
        ...
        let final_path = PathBuf::from("sabs").join(format!("{}.wasm", crate_name));
        fs::copy(&wasm_path, &final_path)?;

        tracing::info!(
            "AutoFabricator: Successfully fabricated SAB -> {}",
            final_path.display()
        );
    ...
    pub async fn fabricate_native(&self, crate_name: &str) -> Result<PathBuf> {
        let work_dir = PathBuf::from("fabrication_native").join(crate_name);
        if work_dir.exists() {
            fs::remove_dir_all(&work_dir)?;
        }
        ...
        self.copy_dir(&PathBuf::from("native_template"), &work_dir)?;
        ...
        let prompt = format!(
            ...
            3. Wrap basic features of {} inside aas_process",
        ...
        cargo_toml = cargo_toml.replace(
            "universal-native-template",
            &format!("{}-native", crate_name),
        );
        fs::write(&cargo_path, cargo_toml)?;

        if let Some(rust_code) = rust_str
            && rust_code.contains("aas_process")
        {
            let lib_path = work_dir.join("src").join("lib.rs");
            fs::write(&lib_path, rust_code)?;
        }
        ...
        let final_path = PathBuf::from("sabs").join(format!("{}.dll", crate_name));
        fs::copy(&dll_path, &final_path)?;

        tracing::info!(
            "AutoFabricator: Successfully fabricated NATIVE enzyme -> {}",
            final_path.display()
        );
    ...
    fn extract_section(&self, text: &str, start_tag: &str, end_tag: &str) -> Option<String> {
        if let Some(start) = text.find(start_tag)
            && let Some(end) = text[start + start_tag.len()..].find(end_tag)
        {
            return Some(
                text[start + start_tag.len()..start + start_tag.len() + end]
                    .trim()
                    .to_string(),
            );
        }
