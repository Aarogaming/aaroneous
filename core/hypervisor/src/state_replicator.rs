...
impl StateReplicator {
    ...
    pub fn new(node_id: &str, peers: Vec<String>, replication_factor: usize) -> Self {
        println!("[StateReplicator] Initialized for node {} with factor {}", node_id, replication_factor);
        
        Self {
            node_id: node_id.to_string(),
            peers,
            replication_factor: replication_factor.max(1).min(5),
    ...
    pub fn replicate_state(&mut self, state_data: &[u8]) -> Result<String, String> {
        ...
        println!("[StateReplicator] Replication initiated for snapshot {} to {} peers", 
            snapshot_id, self.peers.len());
    ...
    pub fn receive_replication(&mut self, snapshot: StateSnapshot) -> Result<(), String> {
        ...
        let mut replica_state = Arc::new(RwLock::new(snapshot.state_data.clone()));
        ...
        println!("[StateReplicator] Received replication from {} (seq: {})", 
    ...
    pub fn record_ack(&mut self, snapshot_id: &str, node_id: &str, status: ReplicationStatus) {
        if let Some(window) = self.replication_windows.get_mut(snapshot_id) {
            ...
            let confirm_count = window.acks.values()
                .filter(|a| a.status == ReplicationStatus::Confirmed)
                .count();
            
            println!("[StateReplicator] Ack from {} on snapshot {} ({}/{} confirmed)", 
    ...
    fn check_replication_complete(&mut self, snapshot_id: &str) {
        if let Some(window) = self.replication_windows.get(snapshot_id) {
            let confirmed = window.acks.values()
                .filter(|a| a.status == ReplicationStatus::Confirmed)
                .count();
            ...
            if confirmed >= needed {
                println!("[StateReplicator] Replication snapshot {} COMMITTED ({}/{})", 
    ...
    pub fn failover(&mut self, failed_primary: &str) -> Result<Vec<u8>, String> {
        ...
        let best_replica_key = self.replica_states.keys()
            .filter(|k| k.contains(&self.node_id))
            .next()
            .ok_or("No replica available for failover")?
            .clone();
        
        let replica_state = self.replica_states.get(&best_replica_key)
        ...
        println!("[StateReplicator] FAILOVER: Promoted replica from {} (new primary: {})", 
    ...
    pub fn get_statistics(&self) -> ReplicationStatistics {
        ...
        let completed = self.replication_windows.iter()
            .filter(|(_, w)| {
                let confirmed = w.acks.values()
                    .filter(|a| a.status == ReplicationStatus::Confirmed)
                    .count();
                confirmed as usize >= (self.replication_factor as f32 * 0.5).ceil() as usize
            })
            .count();
