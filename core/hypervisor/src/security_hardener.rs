...
impl InputValidator {
    ...
    pub fn new(max_input_size: usize) -> Self {
        println!(
            "[InputValidator] Initialized (max_size: {} bytes)",
            max_input_size
        );
    ...
    fn validate_against_rule(&self, value: &str, rule: &ValidationRule) -> ValidationResult {
        ...
        if let Some(pattern) = &rule.pattern
            && !self.matches_pattern(value, pattern)
        {
    ...
    fn matches_pattern(&self, value: &str, pattern: &str) -> bool {
        match regex::Regex::new(pattern) {
            ...
            Err(_) => true, // Invalid regex — don't reject on regex errors
        }
    }
...
impl RateLimiter {
    ...
    pub fn get_stats(&self) -> RateLimiterStats {
        ...
        let over_limit = buckets.values().filter(|b| b.tokens <= 0.0).count() as u32;
...
impl SecurityHardener {
    ...
    pub fn get_security_report(&self) -> SecurityReport {
        ...
        let rate_limiter_stats: Vec<_> = self
            .rate_limiters
            .values()
            .map(|rl| rl.get_stats())
            .collect();
...
mod tests {
    ...
    fn test_injection_detection() {
        ...
        // Add a rule that rejects SQL injection patterns
        let rule = ValidationRule {
            field_name: "test".to_string(),
            field_type: FieldType::String,
            min_length: 0,
            max_length: 1024,
            pattern: Some(r#"(?i)(DROP|DELETE|INSERT|UPDATE|SELECT|;|--)?"#.to_string()),
            allowed_values: None,
        };
        validator.add_rule(rule);
