...
impl AdaptiveLearningOptimizer {
    ...
    pub fn new(initial_lr: f64, min_lr: f64, max_lr: f64) -> Self {
        println!(
            "[LearningRateOptimizer] Initialized with LR={:.6}, min={:.6}, max={:.6}",
    ...
    pub fn record_loss(&mut self, loss: f64) {
        ...
        self.learning_rate_history
            .push_back(self.current_learning_rate);
    ...
    pub fn optimize(&mut self) -> (LearningStrategy, f64) {
        ...
        if new_strategy != self.strategy {
            println!(
                "[LearningRateOptimizer] Strategy change: {:?} → {:?}",
        ...
        if (new_lr - old_lr).abs() > 1e-8 {
            println!(
                "[LearningRateOptimizer] LR adjusted: {:.6} → {:.6} ({:?})",
                old_lr, new_lr, trend
            );
        }
    ...
    fn analyze_trend(&self) -> LossTrend {
        ...
        let window_count = (recent_losses.len() - 1) as f32;
        if improving_count as f32 / window_count > 0.7 {
            LossTrend::Improving
        } else if oscillating_count as f32 / window_count > 0.6 {
            LossTrend::Oscillating
        } else if diverging_count as f32 / window_count > 0.5 {
            LossTrend::Diverging
        } else {
            LossTrend::Stagnant
        }
    ...
    fn apply_strategy(&self, strategy: LearningStrategy) -> f64 {
        match strategy {
            LearningStrategy::Accelerate => {
                (self.current_learning_rate * self.adjustment_factor).min(self.max_learning_rate)
            }
            LearningStrategy::Maintain => self.current_learning_rate,
            LearningStrategy::Decelerate => {
                (self.current_learning_rate / self.adjustment_factor).max(self.min_learning_rate)
            }
    ...
    pub fn get_convergence_metrics(&self) -> ConvergenceMetrics {
        ...
        // Estimate convergence rate (improvements per step)
        let convergence_rate = if losses.len() >= 2 {
            ((losses[0] - losses[losses.len() - 1]) / losses[0] * 100.0) / (losses.len() - 1) as f64
        } else {
            0.0
        };
