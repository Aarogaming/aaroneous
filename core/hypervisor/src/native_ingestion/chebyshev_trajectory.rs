...
pub fn fit_chebyshev(waypoints: &[ScreenCoordinate]) -> [f32; 6] {
    ...
    let (cx0, cx1, cx2) = if det.abs() > 1e-12 {
        ...
        let det_c0 = bx0 * (s2 * s4 - s3 * s3)
                   - s1 * (bx1 * s4 - bx2 * s3)
                   + s2 * (bx1 * s3 - bx2 * s2);
        let det_c1 = s0 * (bx1 * s4 - bx2 * s3)
                   - bx0 * (s1 * s4 - s2 * s3)
                   + s2 * (s1 * bx2 - s2 * bx1);
        let det_c2 = s0 * (s2 * bx2 - s3 * bx1)
                   - s1 * (s1 * bx2 - s2 * bx1)
                   + bx0 * (s1 * s3 - s2 * s2);
    ...
    let (cy0, cy1, cy2) = if det.abs() > 1e-12 {
        let det_c0 = by0 * (s2 * s4 - s3 * s3)
                   - s1 * (by1 * s4 - by2 * s3)
                   + s2 * (by1 * s3 - by2 * s2);
        let det_c1 = s0 * (by1 * s4 - by2 * s3)
                   - by0 * (s1 * s4 - s2 * s3)
                   + s2 * (s1 * by2 - s2 * by1);
        let det_c2 = s0 * (s2 * by2 - s3 * by1)
                   - s1 * (s1 * by2 - s2 * by1)
                   + by0 * (s1 * s3 - s2 * s2);
        (det_c0 / det, det_c1 / det, det_c2 / det)
    } else {
...
mod tests {
    ...
    fn test_linear_trajectory() {
        let waypoints: Vec<ScreenCoordinate> = (0..10).map(|i| {
            let t = i as f32 / 9.0;
    ...
    fn test_constant_trajectory() {
        let waypoints: Vec<ScreenCoordinate> = (0..5).map(|_| {
            ScreenCoordinate { x: 0.5, y: 0.3 }
        }).collect();
    ...
    fn test_parabolic_trajectory() {
        let waypoints: Vec<ScreenCoordinate> = (0..10).map(|i| {
            let t = i as f32 / 9.0;
            ScreenCoordinate { x: t * t, y: 1.0 - t * t }
        }).collect();
        ...
        for i in 0..10 {
            let t = i as f32 / 9.0;
            let pt = evaluate_chebyshev(&coeffs, t);
            assert!((pt.x - t * t).abs() < 0.02, "x at t={}: {} vs {}", t, pt.x, t * t);
        }
    }
}
