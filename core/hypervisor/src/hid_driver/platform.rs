...
mod windows_ffi {
    ...
    unsafe extern "system" {
        ...
        pub fn mouse_event(dwFlags: u32, dx: u32, dy: u32, dwData: u32, dwExtraInfo: *mut c_void);

        /// Generates a keyboard event
        pub fn keybd_event(bVk: u8, bScan: u8, dwFlags: u32, dwExtraInfo: *mut c_void);
...
pub fn create_platform_backend() -> Result<Arc<dyn HidPlatform>, String> {
    ...
    #[cfg(all(not(test), not(any(target_os = "windows", target_os = "linux"))))]
    {
...
impl WindowsHidPlatform {
    ...
    fn execute_key_press(key: u32, modifiers: u8) -> Result<(), String> {
        ...
        unsafe {
            // Apply modifiers
            if (modifiers & 0x01) != 0 {
                keybd_event(0x11, 0, 0, std::ptr::null_mut());
            } // Ctrl
            if (modifiers & 0x02) != 0 {
                keybd_event(0x10, 0, 0, std::ptr::null_mut());
            } // Shift
            if (modifiers & 0x04) != 0 {
                keybd_event(0x12, 0, 0, std::ptr::null_mut());
            } // Alt
            ...
            // Release modifiers
            if (modifiers & 0x04) != 0 {
                keybd_event(0x12, 0, KEYEVENTF_KEYUP, std::ptr::null_mut());
            } // Alt
            if (modifiers & 0x02) != 0 {
                keybd_event(0x10, 0, KEYEVENTF_KEYUP, std::ptr::null_mut());
            } // Shift
            if (modifiers & 0x01) != 0 {
                keybd_event(0x11, 0, KEYEVENTF_KEYUP, std::ptr::null_mut());
            } // Ctrl
        }
...
#[cfg(test)]
pub struct TestHidPlatform;

#[cfg(test)]
#[async_trait::async_trait]
impl HidPlatform for TestHidPlatform {
    async fn execute_command(&self, cmd: &HidCommand) -> Result<HidResponse, String> {
        tokio::time::sleep(std::time::Duration::from_micros(10)).await;

        match cmd {
            HidCommand::GetCursorPos => Ok(HidResponse::CursorPos { x: 100, y: 100 }),
            HidCommand::QueryKeyState { .. } => Ok(HidResponse::KeyState { pressed: false }),
            _ => Ok(HidResponse::Success),
        }
    }
}
