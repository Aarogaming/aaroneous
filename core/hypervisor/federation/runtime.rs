...
use crate::federation::specialist::{
    Specialist, SpecialistId, SpecialistContext, Decision, ExecutionResult,
};
...
impl ModelManager {
    ...
    fn evict_lru(&mut self, needed_space: u32) -> Result<(), String> {
        ...
        if let Some(id) = lru_id {
            let size = self.loaded_models[&id].size_mb;
...
mod tests {
    use super::*;
    use crate::federation::bootstrap::{DeploymentTarget, Manifest};
