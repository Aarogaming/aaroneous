...
use crate::federation::forge::{
    read_gguf, ForgeRecipe, SplicingSegment,
};
...
/// use std::path::Path;
/// use a_run::federation::tensor_vault::{TensorVault, VaultQuery};
///
/// let mut vault = TensorVault::new();
/// vault.index_model(Path::new("/path/to/models/Mistral-7B-Instruct-v0.3-Q4_K_M.gguf")).await?;
/// vault.index_model(Path::new("/path/to/models/Meta-Llama-3.1-8B-Instruct-Q4_K_M.gguf")).await?;
///
...
pub fn recipe_from_dna_compare(
    ...
    let _path_a = PathBuf::from(&dna_a.model_path);
    let _path_b = PathBuf::from(&dna_b.model_path);
