...
mod tests {
    use crate::federation::{
        Specialist, SpecialistId, SpecialistContext, SpecialistConfig, Decision,
    ...
    async fn test_proposal_submission_to_bus() {
        ...
        bus.submit_proposal(proposal.clone()).await.unwrap();
    ...
    async fn test_sentinel_arbitration_flow() {
        ...
        bus.submit_proposal(p1).await.unwrap();
        bus.submit_proposal(p2).await.unwrap();
    ...
    async fn test_federation_end_to_end_flow() {
        ...
        sentinel.communication_bus.submit_proposal(p1).await.unwrap();
        sentinel.communication_bus.submit_proposal(p2).await.unwrap();
