...
async fn list_routines() -> impl IntoResponse {
    ...
    let routines_dir = WorkspacePaths::discover().routines();
...
async fn run_routine(Path(id): Path<String>) -> impl IntoResponse {
    let routine_path = WorkspacePaths::discover().routines().join(&id);
...
async fn list_models() -> impl IntoResponse {
    let search_paths = [
        WorkspacePaths::discover().models(),
...
async fn reload_dynamic_specialists(
    ...
    let registry_path = WorkspacePaths::discover().specialist_registry().as_path();
...
async fn distillation_plan(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let plans = tokio::task::spawn_blocking(|| {
        let models_dir = WorkspacePaths::discover().models();
        let data_dir   = WorkspacePaths::discover().training_data();
...
async fn distillation_generate(
    ...
    #[allow(unreachable_code)]
    {
        ...
        let llm_config = LLMConfig {
            ...
            gguf_model_path: Some(WorkspacePaths::discover().models().join("foundation_v1.gguf")),
        ...
        let llm = match LLMClient::new(llm_config).await {
            ...
            Err(e) => return (
                ...
                Json(serde_json::json!({
                    ...
                    "path": WorkspacePaths::discover().models().join("foundation_v1.gguf").to_string_lossy(),
                })),
            ).into_response(),
        };
        ...
        tokio::task::spawn(async move {
            ...
            let training_data_dir = WorkspacePaths::discover().training_data().as_path();
...
async fn distillation_analyze(
    ...
    let model_path = WorkspacePaths::discover().sovereign_model(&sovereign);
    ...
    match result {
        Ok(Ok((analysis, sov))) => {
            ...
            Json(serde_json::json!({
                "ok": true,
                "sovereign": sovereign,
                "model_path": WorkspacePaths::discover().sovereign_model(&sovereign).to_string_lossy(),
                "analysis": {
...
async fn dna_dissect(
    ...
    let model_path = if std::path::Path::new(&req.model).is_absolute() {
        ...
        std::path::WorkspacePaths::discover().models().join(, req.model))
    };
...
async fn dna_genome(
    ...
    let model_path = std::path::WorkspacePaths::discover().models().join(, filename));
...
async fn dna_compare(
    ...
    let resolve = |name: &str| -> std::path::PathBuf {
        ...
        std::path::WorkspacePaths::discover().models().join(, filename))
    };
...
async fn dna_roster(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    let models_dir = WorkspacePaths::discover().models().as_path();
    let known_models = [
...
async fn models_registry(State(_state): State<AppState>) -> impl IntoResponse {
    ...
    Json(serde_json::json!({
        ...
        "models_dir":     WorkspacePaths::discover().models().to_string_lossy(),
        "inbox_dir":      WorkspacePaths::discover().models_inbox().to_string_lossy(),
        "models":         models,
...
async fn specialists_export(
    ...
    let models_dir = WorkspacePaths::discover().models().as_path();
    let export_dir = WorkspacePaths::discover().exports();
...
pub async fn omni_constellation(
    ...
    let mut model_path = WorkspacePaths::discover().models().join(, model_name));
    if !model_path.exists() {
