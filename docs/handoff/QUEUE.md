...
- [ ] (open, unclaimed) .github/workflows/ingestion.yml is broken: references sdk/python/ and scripts/ingest.ps1, both removed by an earlier wave. Needs either restoration of what it checks or removal/rewrite now that those paths are gone (possibly folded into the aaroneous-devtools migration instead).
- [ ] (open, unclaimed — believed to be the other session's active track per its own stated plan) Wave 9: zero-warning remediation (deprecated `into_path()` calls in orchestrator, unused vars/imports in compute/hypervisor).
- [ ] (open, unclaimed — believed to be the other session's active track per its own stated plan) Wave 10: hot-path `.unwrap()` audit in ipc_bus, **excluding swmr_shm.rs's busy-claim/open_or_create paths** (claimed above) — machine_packet.rs and any other ipc_bus hot-path unwraps are open.
- [ ] (open, unclaimed — believed to be the other session's active track per its own stated plan) Expand aaroneous-devtools migration: sdk/python/, data/extensions/python/ and win32_intercept/, standalone PowerShell deployment utilities (install.ps1, package_release.ps1, uninstall.ps1).
