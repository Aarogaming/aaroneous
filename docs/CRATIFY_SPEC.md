# Cratify Specification & Compiler Invariant Governance
...
|---|---|---|---|
| Ambient `std::env::*` | Nondeterminism, security leakage | Cratify AST check & compiler ban | Exit Code 1 (Hard Block) |
| Manual `unsafe impl Pod/Zeroable` | Undefined behavior, invalid bit patterns | Cratify AST rule | Exit Code 1 (Hard Block) |
...
| `unsafe transmute` on statics/unaligned data | Instant UB and memory corruption | Cratify audit | Exit Code 1 (Hard Block) |
...
| Workspace prefix stutter (`aaroneous_*`) | Namespace pollution and redundancy | Cratify naming lint | Exit Code 1 (Hard Block) |
...
- **Enforcement**: `cratify::rules::no_ambient_authority`.
- **Capability Replacement**: All paths, tokens, and endpoints must be passed via typed config structs (`paths::WorkspacePaths`, `paths::FederationConfigRegistry`).
