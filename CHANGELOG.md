...
### DevTools Capability Scouting, Cratify Compliance, and MCP Extraction [2026-09-25]
- **fix(devtools): sandbox ambient-workspace tests in `adaptation_engine`**: `DevToolsEngine`, `SelfRebuildEngine`, and `ShadowSandbox` (backing the Fabricator/DevTools specialist) had six unit tests that called `::default()`, resolving their working directory via ambient `paths::WorkspacePaths::discover()` against the real host tree — `AGENTS.md` §2 requires an explicit `tempfile::tempdir()` fixture instead. Four of `ShadowSandbox`'s tests additionally *wrote* into the real host's cache directory, not just read from it. All six now construct explicit test fixtures via `::new(temp.path())`/`with_dir(temp.path())`.
- **fix(devtools): removed a panic-guaranteed `Default` impl**: `SelfRepairEngine`'s `Default` called the fallible `Self::new()`, and on failure retried the *exact same* fallible `ShadowSandbox::new()` call and `.unwrap()`'d it — guaranteed to panic whenever the primary path failed, since the retry changed nothing about the outcome (`AGENTS.md` §5, no unhandled panics on production error-handling paths). Had zero callers anywhere in the workspace (the one real call site already used `SelfRepairEngine::new()?`); removed the impl outright.
- **feat(capabilities): `workspace.duplicate_test_names` tool**: continues the `workspace.health_audit` (PR #44) capability-catalog-growth pattern — a pure, no-subprocess `UniversalTool` (opcode `0x0801`) scanning every `.rs` file under `crates/`, `core/`, `dev/` for `#[test]`/`#[tokio::test]` function names and flagging exact-name collisions across files, since `cargo test <name>` matches by substring across every test binary in the workspace. First run against this repository found **78 collisions**, two of which turned out to be real, fixable defect clusters (below) rather than noise.
- **fix(hypervisor): removed a stale pre-split duplicate test file**: `core/hypervisor/src/core_integration_tests.rs`'s own header said "Consolidated from Phase I and Phase II tests" — it was a 675-line hand-merge of `phase1_integration_tests.rs`+`phase2_integration_tests.rs` that predated their split and was never deleted once it landed, still running on every `cargo test`. 22 of its 25 tests were exact-name matches with byte-identical (or cosmetic-only) bodies in the split files; its 3 "unique" tests contained zero `assert!`/`assert_eq!` calls (`println!`-only ASCII-art status banners that pass unconditionally). Deleted; `cargo test -p hypervisor --lib` unaffected coverage. Resolved 22 of the 78 collisions.
- **refactor: extracted `mcp_service` from `core/hypervisor` into `crates/mcp_server`**: `crates/mcp_server` was a workspace member with no `[[bin]]` target and zero in-workspace references — a much-earlier, smaller duplicate of hypervisor's live `mcp_service` (e.g. `service.rs`: 343 lines there vs. 1612 in hypervisor). The straightforward fix is deleting the stale copy, but `AGENTS.md`'s own component topology puts `core/hypervisor` as "Headless microkernel host & execution loop" and presentation/protocol concerns in separate crates, and the architecture docs (`docs/architecture/MASTER_ARCHITECTURE.md`, `architecture_overview.md`) already described `crates/mcp_server` as the real Ring-3 MCP crate — deleting it would have permanently fused an HTTP/SSE/JSON-RPC protocol server into the kernel crate instead of finishing a separation the project's own docs said was already the plan. Instead, in two verified steps: (1) introduced an `IntentBackend` trait so `McpService` stopped reaching directly into `Federation`'s fields (`dynamic`, `results`, `biology`, specialist internals) in ~15 places, with `impl IntentBackend for Federation` in `core/hypervisor/src/federation/cluster/mcp_backend.rs`; verified behavior-preserving (`cargo test -p hypervisor --lib` unchanged at 1117 tests) before moving anything. (2) moved `mcp_service` (now naming only the trait, never `Federation` — required, not a style choice: Cargo forbids the circular dependency any other direction would create, since hypervisor's own binary needs `McpService`/`HttpServer`) into `crates/mcp_server`, deleted that crate's other unrelated dead stubs (`action_executor.rs`, `capability_broker.rs`, `decision_engine.rs`, `micro_vm.rs`, `mcp_bridge/`), wired `hypervisor`'s `Cargo.toml` to depend on `mcp_server`. Result: exactly one copy of this code, living where the architecture docs already said it should — no doc corrections needed, they were aspirationally accurate and are now simply accurate. Resolved 17 of the 18 `mcp_server`/hypervisor collisions (78 → 56 → 39 total remaining, catalogued as informational in `docs/TECH_DEBT_TEST_DUPLICATION.md`).
- Side findings flagged, not fixed (see `docs/TECH_DEBT_TEST_DUPLICATION.md` for full detail): `phase1_integration_tests.rs`/`phase2_integration_tests.rs` themselves contain zero real assertions (`println!`-only narrative tests, a larger separate test-quality gap); `run_mcp_pipeline` (the `hypervisor mcp` CLI subcommand) has never actually attached a live `Federation` to the `McpService` it starts, so its `ask_*` tools always hit the mock-LLM fallback in production today (pre-existing, confirmed unchanged by this work).
- Verified throughout: `cargo test -p adaptation_engine` (113 passed), `cargo test -p capabilities` (36 passed), `cargo test -p mcp_server` (17 passed + 1 doctest), `cargo test -p hypervisor --lib` (1100 passed, 0 failed, 1 ignored — exactly 1117 minus the 17 tests that moved with the code), `cargo check --workspace --all-targets` (clean, including `studio_hud`/`runtime_monitor`, the two other crates already depending on hypervisor as a library), `cargo fmt -- --check` (clean), `cargo run -p ast_auditor -- audit crates/adaptation_engine/ crates/capabilities/ core/hypervisor/ crates/mcp_server/` (0 violations).

### Dependency Maintenance [2026-09-19]
- **fix(compute): cranelift-codegen 0.116 → 0.135, matched cranelift-frontend to the same release**: Dependabot's bump of `cranelift-codegen` alone (to 0.135.2) left `cranelift-frontend` pinned to 0.116, a version-lockstep break between two sibling crates from the same upstream project — `cranelift-frontend`'s `FuncInstBuilder` (built against 0.116's `InstBuilderBase`) no longer satisfied 0.135's incompatible redefinition of the same trait, so every `builder.ins().<op>()` call in `crates/compute/src/cranelift_jit.rs` failed to resolve. Bumped `cranelift-frontend` to `0.135` (a real, non-yanked, compatible release exists) to restore the matched pair, then followed the resulting compiler errors through three further real API changes in 0.135: `MemFlags::trusted()` moved to `MemFlagsData::trusted()` (`InstBuilder::load`/`store` now take `impl Into<MemFlagsData>` directly rather than a pre-interned `MemFlags` handle), `FunctionBuilder::finalize()` now takes an explicit `TargetFrontendConfig` (available via `self.isa.frontend_config()`), and `iadd_imm`/`ushr_imm` are deprecated in favor of explicit `_s`/`_u` sign/zero-extension variants (replaced with the behavior-preserving choice per each call site's prior extension semantics, confirmed against the generated instruction builder source). No behavioral change intended; the three `cranelift_jit` tests that actually JIT-compile and execute native machine code (`test_cranelift_jit_compilation_and_execution`, `_tensordot_multiplication`, `_microkernel_payload_generation`) all still pass, which is the strongest available evidence the flag/ABI migration is semantically correct, not just syntactically compiling. Verified: `cargo test -p compute --lib` (168/168 pass), `cargo clippy -p compute -- -D warnings` (clean, matching CI's exact `--workspace` gate command — note this does *not* cover `tests/` integration-test targets, where two pre-existing, unrelated clippy findings in `rls_boundary_tests.rs`/`rls_numerical_soak.rs` predate this change and are out of scope here), `cargo fmt -p compute -- --check` (clean), `cargo check --workspace` (clean), `cargo run -p ast_auditor -- audit crates/compute` (0 violations, gate passes).

### CI Fix [2026-09-18]
- **fix(ci): Security Audit workflow reported false failures despite finding zero vulnerabilities**: `.github/workflows/security-audit.yml`'s `cargo-audit` job had no explicit `permissions:` block, so its default `GITHUB_TOKEN` lacked `checks: write`; `rustsec/audit-check@v2` then failed with `Resource not accessible by integration` while trying to publish its results as a check run, even on a run where `cargo audit` itself found `"vulnerabilities":{"found":false,"count":0}` (only 4 pre-existing informational warnings: 3 unmaintained crates, 1 unsound-but-unused `cgmath` API). Confirmed this had been silently red on every `main` push and PR since at least the prior two merges. Added `permissions: { contents: read, checks: write }` to the job, matching the existing pattern in `compliance-review.yml`/`compliance-sweep.yml`. No source or dependency change; CI configuration only.

### Substrate Hardening [2026-09-18]
- **fix(ipc_bus): `LOG_ENTRY_HEADER_SIZE` truncated every persisted `generation` field to zero**: `intent_log.rs`'s log-entry header size constant was hand-written as `48`, four bytes short of `LogEntryHeader`'s real 56-byte size once `align(8)` padding before the trailing `generation: u64` field is accounted for (empirically confirmed via `size_of`). Both the write path (`slice::from_raw_parts`) and every read path (`ptr::copy_nonoverlapping` in `IntentLog::calculate_write_offset`, `LogReader::get_entry`, `LogEntryIter::next`) only ever touched the first 48 of 56 bytes, so `generation` was silently dropped on write and always read back as `0` — undetected because no existing test asserted it round-tripped. Fixed by computing the constant from `size_of::<LogEntryHeader>()`, matching the correct pattern this crate's own `MachinePacket::HEADER_SIZE` already used. Added `test_log_entry_header_size_matches_struct_layout` (locks the constant to the struct) and `test_intent_log_round_trips_nonzero_generation` (end-to-end proof).
- **hardening(wire, ipc_bus): unsafe-code governance gaps closed**: `crates/wire` was missing the `#![deny(unsafe_code)]` AGENTS.md requires for standard domain crates (had zero unsafe usage; now enforced). `crates/ipc_bus` had 14 unsafe blocks across `intent_log.rs`, `shared_memory.rs`, `specialist_bus.rs`, and `swmr_shm.rs` with no `// SAFETY:` rationale and no crate-level unsafe policy at all — added documented rationale to all of them. Eliminated 2 more entirely in `machine_packet.rs` by switching `MachinePacket::as_bytes`/`from_bytes` from manual raw-pointer casts to the crate's own already-established safe `bytemuck::bytes_of`/`try_from_bytes` idiom (matching `from_slice_in_place` one function above). Deliberately did **not** add `#![warn(unsafe_code)]` to `ipc_bus` despite AGENTS.md naming it for "performance/kernel" crates: this repo's mandatory `cargo clippy -- -D warnings` gate promotes `warn`-level lints to hard errors, making it functionally identical to `#![deny(unsafe_code)]` and unusable for a crate with legitimate mmap/lock-free unsafe needs — documented at the top of `ipc_bus/src/lib.rs`.
- **fix(ast_auditor): wired up a governance check that was written but never enforced**: `SafetyCommentVisitor`/`SafetyCommentViolation` (in `rules/safety_comments.rs`) existed with its own `Display` impl but was never re-exported, never added to `UnifiedAuditReport`, and never invoked in `audit_source_file` — `AGENTS.md`'s "documented `// SAFETY:` comments" requirement had zero enforcement. Wired it in fully; running it against the whole workspace surfaced 108 pre-existing undocumented-unsafe violations (now 90 after the ipc_bus/wire fixes above), scattered across `core/hypervisor`, `compute`, `orchestrator`, `platform_bridge`, `si_ir`, and `studio_hud` — out of scope for this change. Left `safety_comment_violations` out of `UnifiedAuditReport::has_failures()` for now (collected and printed, not yet gating), mirroring `deny.toml`'s own precedent of landing a check as a surfaced count before tightening it to `deny` once existing debt is paid down; follow-up to flip it on is logged in `docs/handoff/QUEUE.md`. Also fixed the checker's own line-window bug while using it: `node.span().start().line` (1-indexed) was used directly as an index into `str::lines()`'s 0-indexed `Vec`, off by one — documented the actual (narrower than intended) window this produces rather than changing matching behavior mid-PR, and added 4 tests locking in that documented behavior, including a regression guard for the exact off-by-one trap this PR's own first attempt at multi-line `SAFETY:` comments fell into.
- Verified: `cargo test -p ipc_bus -p wire -p ast_auditor --lib` (66 + 4 + 32 pass, including 6 new tests), `cargo clippy --workspace -- -D warnings` (clean, matching CI's exact gate command), `cargo fmt -- --check` (clean), `cargo check --workspace --all-targets` (clean), `cargo run -p ast_auditor -- audit core/ crates/ dev/emulator_harness/` (gate still passes; 90 pre-existing violations elsewhere surfaced but non-blocking, 0 in wire/ipc_bus).

### Capability Catalog Growth [2026-09-17]
- **feat(capabilities): `workspace.health_audit` tool + fix two real orphaned crates it found**: added `WorkspaceHealthTool` (opcode `0x0800`, category `diagnostics`) to `crates/capabilities`' `UniversalTool` registry (10 tools now, up from 9) — reads a workspace's `Cargo.toml`/`Cargo.lock` directly (no `cargo-audit`/`cargo-deny` subprocess, which aren't guaranteed to be on `PATH` outside a dev/CI box) to report crate directories present on disk but declared in neither `workspace.members` nor `workspace.exclude`, and dependencies locked at more than one version simultaneously. Directly operationalizes two defect classes found by hand earlier in this session (the dead `crates/plugin_api`/`crates/hotload` scaffolding; a `libloading` 0.8.9/0.9.0 split) into a reusable, fully self-contained, pure-Rust capability. Running it against this repository immediately found two more real instances: **`dev/canary_legacy_fixture`** (deliberately non-compiling sample source `crates/orchestration_plane`'s `canary_runner`/`canary_run`/`batch_cratify` examples feed to the auditor by file path, never meant to be a workspace crate — added to `workspace.exclude` with a comment explaining why) and **`dev/chaos_injector`** (a genuinely *empty, tracked* `Cargo.toml` — the same silent-content-loss pattern as the `MASTER_ROADMAP.md` corruption fixed earlier this session — despite `docs/forensics/IMMUNE_SYSTEM_SUMMARY.md` documenting it as one of five permanent "dogfooding pillars" runnable via `cargo test -p chaos_injector`; that command has evidently been silently broken since whenever the manifest content was lost). Gave `chaos_injector` a real `Cargo.toml` (`anyhow` dependency, matching its source) and declared it a workspace member, which surfaced two genuine pre-existing bugs in code that had never actually been compiled: a `u128`/`u64` type mismatch comparing `Instant::elapsed().as_millis()` against a `u64` duration, and an unaligned `*mut u32` write through a `[u8; 1024]`-derived pointer with no alignment guarantee (fixed with `write_unaligned`, not a bare cast + `write`, to stay genuinely UB-free rather than just compiling). Added a permanent regression-guard test, `the_real_aaroneous_workspace_has_no_orphaned_crate_dirs`, so this defect class can't silently recur. Verified: `cargo test -p capabilities -p chaos_injector --lib` (25 + 2 pass), `cargo clippy -p capabilities -p chaos_injector --all-targets -- -D warnings` (clean), `cargo fmt -- --check` (clean), `cargo check --workspace` (clean).
- **fix(capabilities): `workspace.health_audit` scan depth, glob support, and a banned `.canonicalize()` call (review round)**: a Codex review found the tool's own defect class had escaped it — `audit_workspace`'s directory scan was single-level only, so a crate nested inside an already-declared member (`crates/runtime_monitor/runtime_monitor_bench`, untouched `cargo new --lib` scaffolding, never declared) was invisible to it. `collect_crate_dirs` now walks recursively (skipping `target/` and dotfile directories), but stops descending once it hits a directory matched by `workspace.exclude` — Cargo's `exclude` already keeps that entire subtree (e.g. `dev/rfc0006_poc/plugins`, its own independent nested workspace) out of resolution, so enumerating its contents as separate undeclared orphans would be noise, not signal. Declared `crates/runtime_monitor/runtime_monitor_bench` as a workspace member (it compiles cleanly; unused boilerplate, not broken). Also added `path_matches_pattern`/`wildcard_component_match` so `members`/`exclude` entries using Cargo's single-level glob syntax (e.g. `"crates/*"`) are matched correctly instead of by brittle exact-string comparison — deliberately not supporting recursive `**` globbing, which this repository doesn't use and whose exact semantics weren't worth guessing at. Fixed the regression test itself using a banned `.canonicalize()` call (`AGENTS.md`'s Domain-Aware Unsafe Permissions rule; the ast_auditor's ambient-authority gate would have rejected it) — replaced with `paths::normalize_path` over the already-absolute `CARGO_MANIFEST_DIR`. Added 4 new tests covering nested-orphan detection, exclude-halts-recursion, and glob matching. Verified: `cargo test -p capabilities --lib` (28/28 pass), `cargo clippy -p capabilities --all-targets -- -D warnings` (clean), `cargo fmt -- --check` (clean), `cargo check --workspace --all-targets` (clean).

### RFC-0006 Proof-of-Concept [2026-09-17]
- **poc(plugins): RFC-0006 command-buffer ABI proof-of-concept**: added `dev/rfc0006_poc/` (standalone, not `crates/api`/`studio_hud` code — RFC-0006 requires "no `studio_hud` code changes until a proof-of-concept is built and reviewed") demonstrating all five Section 8 acceptance criteria for `docs/rfcs/RFC-0006-PLUGIN_LIFECYCLE_AND_STABLE_UI_CARTRIDGE_ABI.md`'s proposed dynamic plugin loading redesign: (1) a well-behaved plugin `cdylib` loads/ticks/unloads cleanly across 20 repeated cycles, (2) a version-mismatched plugin is refused at load with a clear error, (3) an internal-`catch_unwind` plugin panicking inside `plugin_tick` returns a clean `Faulted` result in-process across 5 consecutive panicking ticks, with a separate control-case test confirming (and correctly *not* claiming to independently satisfy this criterion — an earlier draft wrongly described it that way until review caught it) that an uncaught panic instead takes down whatever process hosts it, safely but completely, which is exactly why the mitigation must live in the plugin, (4) a plugin lying about its `command_count`/`payload_len` (claims `u32::MAX` for both; a second case tests them disagreeing) is truncated to the buffer's real, honestly-claimed capacity, never read out of bounds and never replaying stale bytes left over from an earlier tick — verified both end-to-end through a real plugin and via `proptest` fuzzing 2 adversarial-byte-sequence properties directly against the pure decode function, (5) the host-side path is `#![deny(unsafe_code)]` except one isolated, `// SAFETY:`-documented module (`host/src/loader.rs`) per `AGENTS.md`'s Domain-Aware Unsafe Permissions. `plugins/` (5 tiny fixture crates) is deliberately excluded from the root workspace as its own nested `[workspace]`, modeling a plugin as the genuinely separate, independently-compiled artifact it would be in production. Surfaced one correction to RFC-0006 Section 7 during this work: modern stable Rust (since 1.71/`C-unwind`) already aborts safely — not UB — when a panic reaches a plain `extern "C"` boundary uncaught, which also means "the host wraps every FFI call in `catch_unwind`" as currently worded cannot work (the process aborts before the host's wrapper ever runs); the panic-containment mitigation must live inside the plugin's own `extern "C"` entry point, not the host's call site — see `dev/rfc0006_poc/FINDINGS.md` for the full writeup and evidence table. A first review pass (Codex) found and this PR fixed six real issues: the criterion-3 test mischaracterization above, a `lib*.so`-only fixture path lookup that would fail this repo's Windows CI runner (now uses `std::env::consts::DLL_PREFIX`/`DLL_SUFFIX`), `payload_len` being read but not enforced (now bounds parsing alongside the real buffer length), `TickResultRaw` missing the `bytemuck::Pod`/`Zeroable` derives `AGENTS.md` mandates for FFI boundary types, `CommandBufferWriter` truncating text mid-UTF-8-character, and `decode_commands` not gating on `wire_version` before parsing records. Verified: `cargo test -p rfc0006_host -p rfc0006_abi` (16/16 pass), `cargo clippy -p rfc0006_abi -p rfc0006_host --all-targets -- -D warnings` and the same inside `dev/rfc0006_poc/plugins` (both clean), `cargo fmt -- --check` (clean), `cargo check --workspace` (clean, unaffected).

### Security Hardening [2026-09-17]
- **fix(hypervisor): remove dead unauthenticated dynamic-DLL-loading path**: `core/hypervisor/src/lib.rs` carried a `PluginManager`/`init_plugins` pair that scanned a configured directory for `*.dll` files and loaded each one via `hotload::load_module` (`unsafe { libloading::Library::new(path) }`) with **no signature check, no version negotiation, and no sandboxing** — a duplicate of the same unsound pattern `docs/rfcs/RFC-0006-PLUGIN_LIFECYCLE_AND_STABLE_UI_CARTRIDGE_ABI.md` already documents finding and removing from `studio_hud` (SEC-01/SEC-03), left unremediated here. `init_plugins` had zero callers workspace-wide; its only other touchpoint, `inter_agent.rs`'s `PLUGIN_MANAGER.lock()` in `A2AProtocol::set_flag`, discarded the guard immediately (`Future C-ABI hook invocation via symbol table` — never implemented). Removed both, plus the now-unused `crates/plugin_api` (32 lines, zero implementors of its `Plugin` trait) and `crates/hotload` (10 lines, this was its only caller) from the workspace, and their `libloading`/`once_cell` dependency lines from `core/hypervisor/Cargo.toml`. This was dead code with no live call path (not remotely triggerable), but it directly contradicted the 2026-09-05 "CRIT-01: libloading Unchecked DLL Execution (SEC-01)... Remediated" entry below — that remediation evidently missed this instance. The real forward path for dynamic plugin loading is RFC-0006's command-buffer ABI proof-of-concept, still unimplemented (`crates/api` is a 9-line empty shell). Verified: `cargo check --workspace`, `cargo clippy -p hypervisor --lib -- -D warnings`, `cargo fmt -p hypervisor -- --check`, `cargo test -p hypervisor --lib inter_agent` (6/6 pass) all clean.
...
- **fix(hypervisor): iroh 0.98 → 1.2 (`fleet`/`p2p-iroh` features)**: bumps the P2P federation transport, which unblocks four previously-deferred RUSTSEC advisories at once — `iroh` 0.98.2 transitively pinned `hickory-resolver` to an exact vulnerable beta (RUSTSEC-2026-0119, RUSTSEC-2026-0120) and `iroh-relay` capped `lru` below its patched version (RUSTSEC-2026-0253); iroh 1.2 drops the `hickory`/`heapless`/`atomic-polyfill` (RUSTSEC-2023-0089) transitive chain entirely and resolves `lru` to 0.18.4. `core/hypervisor/src/federation/p2p/iroh_node.rs`'s `Endpoint`/`EndpointAddr`/`presets::N0` API surface is unchanged between these versions — no source changes needed, version bump only. Removed the now-stale ignore entries for the four resolved advisories from `deny.toml` and `.cargo/audit.toml`.

### Documentation Fix [2026-09-17]
- **fix(docs): MASTER_ROADMAP.md text corruption**: repaired a systematic single-character-drop corruption (consistent with a broken control-character unescaper — `\a`/`\b`/`\t`/`\n`/`\r`/`\v`-style sequences each consuming one letter, and for `\t`/`\n` also leaving a stray tab or line break) that had silently mangled 25+ terms and all 3 fenced code blocks: `aaroneous`→`aroneous`, `bytemuck`→`ytemuck`, `rkyv`→`kyv`, `tokio`→`okio`, `rig.rs`→`ig.rs`, `ring_buffer.rs`→`ing_buffer.rs`, `nvml-wrapper`→`vml-wrapper`, `nats_client.rs`→`ats_client.rs` (each also missing the `v`-prefix on all 13 phase version numbers, e.g. `v0.4.0`→`0.4.0`), and ` ```bash`/` ``` ` fences collapsed to single backticks. Reconstructed each instance by cross-referencing real workspace terms (`Cargo.toml` dependency names, actual source file paths) rather than guessing; verified zero remaining control-byte anomalies via a full-file byte scan. No semantic/roadmap content changed, only restored legibility. Verified: `cargo run -p ast_auditor -- check-encoding` clean; `git diff --check` clean.
