...
- **Follow-up corrections (same day)**: `core-contracts` reclassified as `kernel`; `core/hypervisor` binaries named as the composition root; profile dependency direction added (CRATIFY_SPEC section 2.3, planned, with `ipc_bus`/`compute` -> `paths` as baseline violations); `no_ambient_authority` status now discloses five library-code `#[allow(ambient_authority)]` exemptions; hot-path marker count corrected from "zero" to four (the auditor also accepts `#[doc = "hot_path"]`); gate audit scope disclosed. Docs: ring rule restated accurately (holds for library crates; hypervisor binaries exempt), 67 `file:///d:/...` links made relative, links to the private devtools `OPERATING_MODEL.md` removed, renamed-file and renamed-crate links fixed (`assimilation_specification`, `autonomic_adaptation`), `dev/README.md` rewritten (it claimed `dev/` was the authoritative source of truth and indexed moved tooling), stale crate/file counts stamped with a commit, duplicate `certify_module_design.md` copies removed (archived copy retained).
