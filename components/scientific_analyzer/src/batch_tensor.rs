...
use crate::verifier::{VerificationResult, Verdict};
...
pub fn prioritize_tests(observations: &[AstObservation], temperature: f64) -> Vec<(usize, f64)> {
    ...
    let mut indexed_probs: Vec<(usize, f64)> = probabilities.iter().enumerate().map(|(i, &p)| (i, p)).collect();
...
pub fn batch_generate_hypotheses(observations: &[AstObservation]) -> Vec<Vec<Hypothesis>> {
    ...
    observations.iter().enumerate().map(|(i, obs)| {
        ...
        if !similar_files.is_empty() {
            hypotheses.push(Hypothesis::new(
                &format!("File {} shares patterns with {} similar files", i, similar_files.len()),
                "duplication",
                0.6,
            ));
        }
...
pub fn batch_run_experiments(
    ...
    hypotheses_batch.iter().map(|hypotheses| {
        hypotheses.iter().map(|h| {
            ExperimentResult::run(h, workspace_root)
        }).collect()
    }).collect()
}
...
pub fn batch_verify(
    ...
    hypotheses_batch.iter().zip(experiments_batch.iter()).map(|(hypotheses, experiments)| {
        hypotheses.iter().zip(experiments.iter()).map(|(h, e)| {
            VerificationResult::verify(h, e)
        }).collect()
    }).collect()
}
...
pub fn run_batch_analysis(
    ...
    let time_series: Vec<Vec<f64>> = feature_matrix.iter().map(|f| f.clone()).collect();
