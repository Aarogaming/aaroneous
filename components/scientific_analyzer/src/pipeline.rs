...
use crate::verifier::{VerificationResult, ConstellationUpdate};
...
impl ScientificPipeline {
    ...
    pub async fn analyze_file(&self, file_path: &Path) -> anyhow::Result<AnalysisReport> {
        ...
        // Generate summary
        let summary = self.generate_summary(&observation, &verifications, start.elapsed().as_secs_f64() * 1000.0);
    ...
    fn generate_summary(
        ...
        let verified = verifications.iter().filter(|v| matches!(v.verdict, crate::verifier::Verdict::Verified)).count();
        let falsified = verifications.iter().filter(|v| matches!(v.verdict, crate::verifier::Verdict::Falsified)).count();
        let unstable = verifications.iter().filter(|v| matches!(v.verdict, crate::verifier::Verdict::Unstable)).count();
        let inconclusive = verifications.iter().filter(|v| matches!(v.verdict, crate::verifier::Verdict::Inconclusive)).count();
        
        let avg_confidence = if verifications.is_empty() {
            0.0
        } else {
            verifications.iter().map(|v| v.posterior_confidence).sum::<f64>() / verifications.len() as f64
        };
