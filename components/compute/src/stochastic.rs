...
pub fn monte_carlo_simulate(input: &[f64], iterations: usize, rng: &mut impl Rng) -> anyhow::Result<Vec<f64>> {
    if input.is_empty() {
        return Ok(vec![]);
    }
    ...
    let sim_std = results.iter().map(|x| (x - sim_mean).powi(2)).sum::<f64>().sqrt();
...
pub fn metropolis_hastings_step(current: f64, proposal_std: f64, rng: &mut impl Rng, log_likelihood: impl Fn(f64) -> f64) -> f64 {
