...
pub fn transfer_entropy(
    ...
    let n_bins = 5;
    let all_values: Vec<f64> = source_history.iter().chain(target_history.iter()).cloned().collect();
    ...
    let source_target_cond_prob: Vec<f64> = source_target_cond_counts.iter().map(|&c| c / total).collect();
    ...
    for y_next in 0..n_bins {
        for y_past in 0..n_bins {
            for x_past in 0..n_bins {
                ...
                let p_y_given_yp_xp = source_target_cond_prob[y_next * n_bins * n_bins + y_past * n_bins + x_past];
