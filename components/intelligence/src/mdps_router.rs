...
impl TaskRoutingEngine {
    ...
    fn calculate_expected_reward(&self, state: &RoutingState, specialist: &Specialist) -> f64 {
        let complexity_match = 1.0 - (state.task_complexity_bin as f64 / 4.0 - specialist.success_rate).abs();
    ...
    pub fn find_optimal_specialist(&mut self, task: &RoutableTask) -> RoutingDecision {
        ...
        let avg_load = if self.specialists.is_empty() {
            ...
            self.specialists.iter().map(|s| 1.0 - s.capacity).sum::<f64>() / self.specialists.len() as f64
        };
    ...
    pub fn update_specialist_performance(&mut self, specialist_id: &str, success: bool, completion_time: f64) {
        if let Some(specialist) = self.specialists.iter_mut().find(|s| s.id == specialist_id) {
            // Update success rate with exponential moving average
            specialist.success_rate = specialist.success_rate * 0.9 + if success { 1.0 } else { 0.0 } * 0.1;
            
            // Update completion time with EMA
            specialist.avg_completion_time = specialist.avg_completion_time * 0.9 + completion_time * 0.1;
