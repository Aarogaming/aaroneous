...
impl SystemBiology {
    ...
    pub fn update_metabolism(&mut self) {
        ...
        for (_, metabolism) in self.specialist_metabolism.iter_mut() {
            ...
            metabolism.tokens = (metabolism.tokens + elapsed_spec * spec_regen).min(metabolism.max_tokens);
    ...
    pub fn set_expression_rate(&mut self, new_rate: f32) {
        let clamped_rate = new_rate.max(0.0).min(1.0);
    ...
    pub fn request_mutation(&self, specialist_id: &str, goal: &str) {
        tracing::warn!("Metabolic Governor: Stagnation detected for {}. Requesting evolution for goal: {}", specialist_id, goal);
...
mod tests {
    ...
    fn test_specialist_registration() {
        ...
        assert!(biology.get_specialist_metabolism("specialist_ariel").is_some());
    }
    ...
    fn test_throttle_state_transitions() {
        ...
        biology.set_expression_rate(0.2);
