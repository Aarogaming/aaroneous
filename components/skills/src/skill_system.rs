...
#[derive(Clone, Copy, Debug, Serialize, Deserialize, PartialEq, Eq, Hash, Display, EnumString, EnumIter)]
pub enum SkillType {
    #[strum(serialize = "DAG")]
...
#[derive(Clone, Copy, Debug, Serialize, Deserialize, PartialEq, Eq, PartialOrd, Ord, Hash, Display, EnumString, EnumIter)]
pub enum SoulRank {
...
impl Skill {
    ...
    pub fn record_usage(&mut self, success: bool, quality: f64) {
        ...
        let new_success_count = if success { prev_success_count + 1 } else { prev_success_count };
...
impl FusedSkill {
    pub fn new(
        ...
        let base_power = parent_skills.iter().map(|s| s.power_score()).sum::<f64>()
            / parent_skills.len() as f64;
...
impl SpecialistSkillSet {
    ...
    pub fn check_rank_evolution(&mut self) -> Option<SoulRank> {
        let skilled_count = self
            .skills
            .values()
            .filter(|s| s.level >= 5)
            .count();

        let awakened_count = self
            .skills
            .values()
            .filter(|s| s.is_awakened)
            .count();
        ...
        let next_rank = match self.soul_rank {
            ...
            SoulRank::Rank3Journeyman => {
                if self.skills.values().filter(|s| s.level >= 10).count() >= 3 && awakened_count >= 1
                {
    ...
    pub fn total_power_score(&self) -> f64 {
        ...
        let fusion_boost: f64 = self
            .fused_skills
            .iter()
            .map(|f| f.power_multiplier)
            .sum();
...
impl SkillRegistry {
    ...
    pub fn create_skillset(&mut self, specialist_id: String) {
        self.specialist_skillsets
            .insert(specialist_id.clone(), SpecialistSkillSet::new(specialist_id));
    }
    ...
    pub fn find_specialists_with_skill(&self, skill_type: SkillType) -> Vec<String> {
        self.specialist_skillsets
            ...
            .filter_map(|(id, skillset)| {
                if skillset.get_skills_by_type(skill_type).len() > 0 {
                    Some(id.clone())
                } else {
                    None
                }
