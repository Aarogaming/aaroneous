        ...
        for i in 0..n {
            for j in 0..n {
                if i == j {
                    ...
                    let js = compute_js_divergence(&embeddings[i].features, &embeddings[j].features);
                    js_divergence_matrix[i][j] = js;
                    js_divergence_matrix[j][i] = js;

                    // Mutual Information (approximated via feature correlation)
                    let mi = approximate_mutual_information(&embeddings[i].features, &embeddings[j].features);
...
pub fn compute_information_flow(embeddings: &[SabEmbedding], time_series: &[Vec<f64>]) -> Vec<Vec<f64>> {
    let n = embeddings.len();
...
pub fn rate_distortion_analysis(embeddings: &[SabEmbedding], target_distortion: f64) -> f64 {
    ...
    for d in 0..dim {
        let mean: f64 = embeddings.iter().map(|e| e.features[d]).sum::<f64>() / embeddings.len() as f64;
        let variance: f64 = embeddings.iter().map(|e| (e.features[d] - mean).powi(2)).sum::<f64>() / embeddings.len() as f64;
...
pub fn find_redundant_surfaces(embeddings: &[SabEmbedding], similarity_threshold: f64) -> Vec<(usize, usize)> {
    let n = embeddings.len();
...
pub fn compute_surface_importance(similarity_matrix: &SabSimilarityMatrix, damping: f64, iterations: usize) -> Vec<f64> {
    let n = similarity_matrix.surface_names.len();
    if n == 0 {
        return vec![];
    }
...
fn approximate_transfer_entropy(source: &[f64], target: &[f64]) -> f64 {
    ...
    let cov: f64 = source.iter().take(n).zip(target.iter().skip(1).take(n))
        .map(|(s, t)| (s - mean_source) * (t - mean_target))
        .sum::<f64>() / n as f64;

    let var_source: f64 = source.iter().take(n).map(|x| (x - mean_source).powi(2)).sum::<f64>() / n as f64;
    let var_target: f64 = target.iter().skip(1).take(n).map(|x| (x - mean_target).powi(2)).sum::<f64>() / n as f64;
...
mod tests {
    ...
    fn create_test_embeddings() -> Vec<SabEmbedding> {
        vec![
            SabEmbedding::new("surface_a", vec![1.0, 0.5, 0.2], SabMetadata {
                artifact_count: 5, pattern_count: 3, module_count: 2, complexity_score: 0.7, last_updated: 100.0,
            }),
            SabEmbedding::new("surface_b", vec![0.9, 0.6, 0.3], SabMetadata {
                artifact_count: 4, pattern_count: 2, module_count: 3, complexity_score: 0.6, last_updated: 90.0,
            }),
            SabEmbedding::new("surface_c", vec![0.1, 0.2, 0.9], SabMetadata {
                artifact_count: 2, pattern_count: 5, module_count: 1, complexity_score: 0.3, last_updated: 80.0,
            }),
        ]
    }
