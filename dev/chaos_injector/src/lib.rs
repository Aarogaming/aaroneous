...
pub fn inject_buffer_overflow() -> Result<()> {
    ...
    for i in 0..2000 {
        ...
        unsafe {
            // SAFETY: `offset + size_of::<u32>() <= buffer.len()` is checked
            // above, so this 4-byte write stays within `buffer`. The pointer
            // is not guaranteed u32-aligned ([u8; N] only guarantees 1-byte
            // alignment), so `write_unaligned` - not `write` - is required.
            let ptr = buffer.as_mut_ptr().add(offset) as *mut u32;
            ptr.write_unaligned(i as u32);
        }
...
mod stress_tests {
    ...
    fn test_cpu_starvation_recovery() {
        ...
        assert!(
            ...
            "System hung after CPU starvation"
        );
    }
