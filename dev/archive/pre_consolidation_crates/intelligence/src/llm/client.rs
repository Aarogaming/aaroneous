use crate::llm::types::{LLMConfig, ProviderType, TaskAnalysis, TaskAnalysisContext};
use anyhow::Result;
...
impl LLMClient {
        ...
        match self.config.provider_type {
            ProviderType::Mock => Ok(TaskAnalysis {
                ...
                recommended_approach: format!(
                    "Analyze {} in domain {}",
                    _context.file_type, _context.specialist_domain
                ),
    ...
    pub async fn generate_domain_response(
        ...
        match self.config.provider_type {
            ProviderType::Mock => Ok(format!("[Mock] Response to: {}", user_prompt)),
            _ => {
                anyhow::bail!("Provider not implemented yet")
            }
        }
    ...
    pub async fn generate_design(
        &self,
        _context: &crate::llm::types::DesignContext,
    ) -> Result<String> {
        match self.config.provider_type {
            ProviderType::Mock => Ok("[Mock] Generated design".to_string()),
            _ => {
                anyhow::bail!("Provider not implemented yet")
            }
        }
