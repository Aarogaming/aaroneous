...
MAX_WORKERS = 8  # 8 simultaneous LM Studio streams
...
def process_file(filepath):
    ...
    try:
        ...
        # Pre-compress and downscale EVERYTHING to 1024x1024 to save VRAM and Base64 transfer time
        with Image.open(filepath) as img:
            if img.mode != "RGB":
                img = img.convert("RGB")
            # LANCZOS is high quality, ensures math symbols stay sharp even at lower res
        ...
        # Keep 20-minute timeout just in case it hits a dense formula
...
def main():
    log("Scanning for JPG files with ULTRA-RESIZE and 8x PARALLEL CHAINING enabled...")
    files = []
