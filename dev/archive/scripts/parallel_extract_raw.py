...
def process_file(filepath):
    ...
    try:
        # STRIPPED OUT PIL (Pillow) ENTIRELY.
        # Sending raw Base64 exactly as the very first script did successfully for 60+ images.
        ...
        return True, f"Extracted: {os.path.basename(filepath)} (RAW)"
    except Exception as e:
        return False, f"Error on {os.path.basename(filepath)}: {str(e)}"


def main():
    log("Scanning for JPG files with RAW-MODE and 4x PARALLEL CHAINING enabled...")
    files = []
