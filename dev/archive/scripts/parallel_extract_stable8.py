...
# ULTRA PERFORMANCE SETTINGS for RTX 5070 Ti + 64GB RAM
MAX_WORKERS = 4  # 8 simultaneous LM Studio streams
MAX_PIXELS = 1024  # Optimal native resolution for most vision models
...
def process_file(filepath):
    ...
    try:
        ...
        # DECOUPLED FILE READ: We read the raw bytes into RAM first, then pass to PIL.
        # This completely eliminates the Windows [Errno 22] file-lock crash caused by 8 threads
        # fighting over disk I/O simultaneously.
        ...
        with Image.open(io.BytesIO(raw_bytes)) as img:
            if img.mode != "RGB":
                img = img.convert("RGB")
            # LANCZOS is high quality, ensures math symbols stay sharp
        ...
        # Keep 20-minute timeout for dense images
...
def main():
    log(
        "Scanning for JPG files with STABILIZED ULTRA-RESIZE and 8x PARALLEL CHAINING enabled..."
    )
    files = []
