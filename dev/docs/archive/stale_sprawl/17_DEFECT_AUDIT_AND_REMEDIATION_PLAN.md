...
- **Root Cause:** `NucleotidePacket` specifies `#[repr(C, align(8))]`. In `NucleotidePacket::from_bytes(bytes: &[u8])`, the slice pointer is cast directly via `bytes.as_ptr() as *const Self` without validating 8-byte boundary alignment (`ptr as usize % 8 == 0`). In Rust, creating a reference to an unaligned pointer is undefined behavior (UB) that triggers CPU alignment faults or torn reads on ARM64 and WASM targets.
...
- **Root Cause:** `SynapseBridge` is a zero-sized struct with an empty `.connect()` method that prints to stdout but does not instantiate or bind to `nervous_system::SharedMemorySynapse`.
- **Remediation:** Wire `SynapseBridge` to open and exchange packets with the active `SharedMemorySynapse`.
...
|:---|:---|:---|:---|:---|
| **#1** | Enforce 8-byte pointer alignment in `NucleotidePacket` | `crates/nervous_system` | v0.3.3 | ⏳ Pending |
