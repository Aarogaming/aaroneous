...
| **`MaelstromUI/`** | Frontend | Tauri 2.0 + React + Vite user interface. | Active (Fixes Needed in `App.tsx`) |
...
| **`shaders/`** | GPU Shaders | WGSL compute shaders (`epigenetic_gate.wgsl`, `reflex_kernel.wgsl`). | Active GPU Kernels |
