...
/// Sovereign Organ Wrapper for `target_util`
#[derive(Debug, Clone)]
pub struct TargetUtilOrgan {
    pub executable_path: String,
    pub domain_opcode: u16,
    pub is_dry_run: bool,
}

impl Default for TargetUtilOrgan {
    fn default() -> Self {
        Self {
            executable_path: "target_util.exe".to_string(),
...
impl TargetUtilOrgan {
    ...
    pub async fn invoke(&self, args: &[&str], input_payload: Option<&[u8]>) -> Result<Vec<u8>> {
        info!(target: "target_util_organ", ?args, "Invoking sovereign wrapped organ");
        ...
        if !output.status.success() {
            ...
            warn!(target: "target_util_organ", exit_code = ?output.status.code(), %err_msg, "Organ execution returned warning/error");
        }
