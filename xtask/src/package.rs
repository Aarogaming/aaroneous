...
pub fn run(args: &[String]) -> Result<()> {
    ...
    let status = Command::new("cargo")
        ...
        .status()?;
    if !status.success() {
        bail!("Failed to build release binaries");
    }
    ...
    for dir in ["config", "shaders", "deploy/mcp_clients"] {
        ...
        let _ = Command::new("powershell")
            .args([
                "-Command",
                &format!(
                    "Copy-Item -Path '{}' -Destination '{}' -Recurse -ErrorAction SilentlyContinue",
                    dir, dest
                ),
            ])
            .status();
    ...
    let status = Command::new("powershell")
        ...
        .status()?;
    if !status.success() {
        bail!("Failed to zip staging dir");
    }
