...
impl SpatialCoord {
    ...
    pub fn semantic_distance_to(&self, other: &SpatialCoord, _clustering_context: &ClusteringContext) -> f64 {
        ...
        let semantic_factor = (domain_proximity * 0.25 + temporal_alignment * 0.25 + priority_alignment * 0.25) * 0.75;
...
impl SpatialBounds {
    pub fn contains(&self, coord: &SpatialCoord) -> bool {
        coord.x >= self.x_min && coord.x <= self.x_max
            && coord.y >= self.y_min && coord.y <= self.y_max
            && coord.z >= self.z_min && coord.z <= self.z_max
    }
}
...
impl Constellation {
    ...
    pub fn find_cluster(&self, node_id: &str) -> Option<&Cluster> {
        self.clusters.iter().find(|c| c.node_ids.contains(&node_id.to_string()))
    }
