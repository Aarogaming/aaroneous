...
use crate::agents::{SpecialistAgent, RelicAgent, create_specialist, create_relic};
...
impl ControlPlane {
    ...
    async fn execute_command(
        ...
        match msg {
            ...
            ControlMessage::HaltSpecialist { name } => {
                self.halt_specialist(&name).await
            }
            ControlMessage::SetExpressionRate { rate } => {
                biology.set_expression_rate(rate);
                (
                    format!("federation.control.response.set_expression_rate"),
                    json!({
                        "success": true,
                        "new_rate": rate,
    ...
    async fn spawn_specialist(
        ...
        if states.contains_key(name) {
            return (
                format!("federation.control.response.spawn_specialist"),
                json!({
                    "success": false,
    ...
    async fn recalibrate_specialist(
        &self,
        name: &str,
        bias: Option<Value>,
    ) -> (String, Value) {
    ...
    async fn adjust_resource_allocation(
        ...
        if states.contains_key(specialist_name) {
            (
                format!("federation.control.response.adjust_resource"),
                json!({
                    "success": true,
