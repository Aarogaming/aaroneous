...
mod tests {
    ...
    async fn test_stress_1000_commands() {
        ...
        // Validation: p99 should be < 3ms (3000us) for responsive HID
        // Note: On real hardware with direct Windows API calls, this should be < 1ms
        // In test environments with simulation, syscall overhead can be higher
        assert!(percentiles.p99 < 3000, 
            "p99 latency {}us exceeds 3ms threshold for HID driver", 
            percentiles.p99);
        
        // p95 should be reasonable (< 2ms on test)
        assert!(percentiles.p95 < 2000,
            "p95 latency {}us exceeds 2ms target",
            percentiles.p95);
    }
    ...
    async fn test_latency_validation_p99() {
        ...
        // Verify p99 is within spec
        // Real hardware target: < 1ms (1000us)
        // Test environment threshold: < 10ms (10000us) - syscall overhead can be significant
        assert!(percentiles.p99 < 10000,
            "p99 latency must be <10ms for marionette control, got {}us",
            percentiles.p99);
        
        println!("Latency validation passed: p99={}us (target <10ms for test env, <1ms on real hw)", percentiles.p99);
    }
}
