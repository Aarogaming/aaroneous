...
/// 3. **Inbox watcher**: drop a `.gguf` into `D:\Aaroneous\models\inbox\` and it
...
/// Scan `D:\Aaroneous\models\` and return all `.gguf` files as `ModelEntry` values.
...
pub fn scan_models_dir() -> Vec<ModelEntry> {
    let models_dir = PathBuf::from("D:\\Aaroneous\\models");
...
pub async fn register_as_specialist(model_path: &Path, tags: &[String]) {
    ...
    let registry_path = PathBuf::from("D:\\Aaroneous\\config\\specialist_registry.json");
...
///
/// Watches `D:\Aaroneous\models\inbox\` every 10 seconds for new `.gguf` files.
/// When a file is found, triggers the full import pipeline:
/// 1. Move to `D:\Aaroneous\models\`
...
pub async fn start_inbox_watcher(jobs: ImportJobs) {
    let inbox_dir = PathBuf::from("D:\\Aaroneous\\models\\inbox");
        ...
        loop {
            ...
            for entry in entries.flatten() {
                ...
                tokio::spawn(async move {
                    ...
                    let spec = dest_clone.to_string_lossy().to_string();
                    {
    ...
    info!("Inbox watcher started: D:\\Aaroneous\\models\\inbox\\");
}
