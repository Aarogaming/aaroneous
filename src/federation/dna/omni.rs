...
use crate::federation::forge::{GgufMeta, MetaValue};
...
impl OmniConstellation {
    ...
    fn establish_orbits(&mut self, new_node: &OmniNode) {
        for (id, existing) in self.nodes.iter_mut() {
...
impl OmniArchiver {
    ...
    pub fn archive_to_gguf(constellation: &OmniConstellation, gguf_path: &Path, output_path: &Path) -> anyhow::Result<()> {
