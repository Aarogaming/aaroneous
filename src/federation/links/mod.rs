/// Aaroneous Link System — integration bridge to external applications.
///
/// A "Link" is a registered integration that Aaroneous pushes events to.
/// Each Link has:
/// - A name and type (Webhook, Discord, Slack, Notion, custom)
/// - An event filter (which sovereign events trigger it)
/// - A delivery target (HTTP endpoint, Discord channel, etc.)
/// - A payload template (how to format the event for that target)
///
...
/// | Discord   | POST rich embed to a Discord webhook URL                  |
/// | Slack     | POST Block Kit message to a Slack incoming webhook        |
/// | Notion    | Append a block to a Notion database (via Integration API) |
/// | Custom    | User-defined template + HTTP target                       |
///
/// # Event routing
///
/// Every `execution_complete`, `intent_submitted`, and `guild_decomposition`
/// SSE event that fires in the federation can trigger registered Links.
/// The `LinkRegistry` runs as a background task that reads from the
/// federation's broadcast channel and dispatches matching links.
///
/// # Storage
///
/// Links are persisted to `D:\Aaroneous\config\links.json` so they survive
/// server restarts. The registry is loaded on startup and saved on any change.
...
const LINKS_CONFIG_PATH: &str = "D:\\Aaroneous\\config\\links.json";

/// Load links from disk (creates empty registry if file doesn't exist).
pub fn load_links() -> Vec<Link> {
    let path = std::path::Path::new(LINKS_CONFIG_PATH);
...
pub fn save_links(links: &[Link]) -> anyhow::Result<()> {
    let dir = std::path::Path::new(LINKS_CONFIG_PATH).parent().unwrap();
    std::fs::create_dir_all(dir)?;
    let json = serde_json::to_string_pretty(links)?;
    std::fs::write(LINKS_CONFIG_PATH, json)?;
    Ok(())
}
