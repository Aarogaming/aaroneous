...
impl AutoFabricator {
    pub fn new(llm: Arc<LLMClient>) -> Self {
        Self {
            llm,
            template_dir: PathBuf::from("D:\\Aaroneous\\templates\\universal_sab"),
            output_dir: PathBuf::from("D:\\Aaroneous\\data\\sabs"),
        }
    }
    
    pub async fn fabricate(&self, crate_name: &str) -> Result<PathBuf> {
        let work_dir = PathBuf::from(format!("D:\\Aaroneous\\data\\fabrication\\{}", crate_name));
        if work_dir.exists() {
        ...
        // 1. Copy template
        self.copy_dir(&self.template_dir, &work_dir)?;
        ...
        let final_path = self.output_dir.join(format!("{}.wasm", crate_name));
    ...
    pub async fn fabricate_native(&self, crate_name: &str) -> Result<PathBuf> {
        let native_template_dir = PathBuf::from("D:\\Aaroneous\\templates\\universal_native");
        let work_dir = PathBuf::from(format!("D:\\Aaroneous\\data\\fabrication\\{}_native", crate_name));
        if work_dir.exists() {
            fs::remove_dir_all(&work_dir)?;
        }
        ...
        self.copy_dir(&native_template_dir, &work_dir)?;
        ...
        let final_path = self.output_dir.join(format!("{}.dll", crate_name));
